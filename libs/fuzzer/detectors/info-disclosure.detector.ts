...
export class InformationDisclosureDetector implements IVulnerabilityDetector {
  ...
  readonly description =
    "Detects sensitive information leakage in responses (CWE-200, CWE-209)";
  ...
  private generateRemediation(
    findings: Array<{ pattern: DisclosurePattern }>,
  ): string {
    ...
    if (
      ...
      remedations.push(
        "Implement proper error handling that returns generic error messages to clients",
      );
    }
    if (
      types.some(
      ...
      remedations.push(
        "Never expose credentials in responses. Use environment variables and secrets managers",
      );
    }
    if (types.some((t) => t.includes("path") || t.includes("internal-ip"))) {
      remedations.push(
        "Sanitize responses to remove internal paths and network information",
      );
    }
    ...
    return (
      remedations.join(". ") ||
      "Review and sanitize all error messages and responses for sensitive data"
    );
  }

  private getPrimaryCwe(
    findings: Array<{ pattern: DisclosurePattern }>,
  ): string {
