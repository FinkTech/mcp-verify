...
export class JwtAttackGenerator implements IPayloadGenerator {
  ...
  readonly description =
    "Generates malicious JWT tokens to test authentication bypass vulnerabilities";
  ...
  private generateClaimAttacks(): GeneratedPayload[] {
    ...
    for (const claims of privilegePayloads) {
      ...
      payloads.push({
        ...
        expectedVulnerableBehavior:
          "Server grants elevated access based on claims",
    ...
    if (this.config.targetIssuer || this.config.targetAudience) {
      payloads.push({
        ...
        expectedVulnerableBehavior:
          "Server accepts token from untrusted issuer",
  ...
  private generateHeaderInjectionAttacks(): GeneratedPayload[] {
    ...
    payloads.push({
      ...
      severity: "critical",
      description:
        "JWT with malicious JKU header pointing to attacker-controlled JWKS",
    ...
    payloads.push({
      ...
      severity: "critical",
      description:
        "JWT with malicious X5U header pointing to attacker certificate",
      expectedVulnerableBehavior:
    ...
    payloads.push({
      ...
      expectedVulnerableBehavior:
        "Server uses attacker-provided key for verification",
    ...
    for (const kid of kidInjections) {
      payloads.push({
        ...
        expectedVulnerableBehavior:
          "Server vulnerable to injection via kid parameter",
  ...
  private generateSignatureAttacks(): GeneratedPayload[] {
    ...
    payloads.push({
      ...
      expectedVulnerableBehavior:
        "Server accepts token without signature section",
      tags: ["authentication", "bypass"],
    });
