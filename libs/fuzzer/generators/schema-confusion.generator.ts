...
import { IPayloadGenerator, GeneratorConfig, GeneratedPayload } from './generator.interface';
...
export class SchemaConfusionGenerator implements IPayloadGenerator {
  ...
  readonly description = 'Generates payloads to exploit type confusion and schema validation gaps';
   ...
   */
  private parseSchema(toolSchema: Record<string, unknown> | null | undefined): ParsedSchema {
    if (!toolSchema || typeof toolSchema !== 'object' || Array.isArray(toolSchema)) {
      throw new Error("Invalid toolSchema: must be a non-null, non-array object.");
    }
    ...
    for (const [propName, propDef] of Object.entries(properties)) {
      if (typeof propDef === 'object' && propDef !== null) {
        parsedProperties[propName] = this.parsePropertySchema(propDef as Record<string, unknown>);
      }
    }
    ...
    const additionalProperties = toolSchema.additionalProperties !== undefined
   ...
   */
  private parsePropertySchema(propDef: Record<string, unknown>): PropertySchema {
    ...
    // Extract constraints
    if (propDef.maxLength !== undefined) schema.maxLength = propDef.maxLength as number;
    if (propDef.minLength !== undefined) schema.minLength = propDef.minLength as number;
    if (propDef.maximum !== undefined) schema.maximum = propDef.maximum as number;
    if (propDef.minimum !== undefined) schema.minimum = propDef.minimum as number;
    if (propDef.pattern !== undefined) schema.pattern = propDef.pattern as string;
    ...
    if (propDef.maxItems !== undefined) schema.maxItems = propDef.maxItems as number;
    if (propDef.minItems !== undefined) schema.minItems = propDef.minItems as number;
    ...
    if (schema.type === 'object' && propDef.properties) {
      ...
      for (const [nestedName, nestedDef] of Object.entries(nestedProps)) {
        if (typeof nestedDef === 'object' && nestedDef !== null) {
          schema.properties[nestedName] = this.parsePropertySchema(nestedDef as Record<string, unknown>);
        }
      }
    ...
    if (schema.type === 'array' && propDef.items) {
      if (typeof propDef.items === 'object' && propDef.items !== null) {
        schema.items = this.parsePropertySchema(propDef.items as Record<string, unknown>);
      }
    }
  ...
  private extractFields(
    ...
    for (const [propName, propSchema] of Object.entries(schema.properties)) {
      ...
      if (propSchema.type === 'object' && propSchema.properties) {
        ...
        const nestedFields = this.extractFields(nestedSchema, currentPath, propSchema.required || []);
  ...
  private boundaryAttacks(field: FieldDescriptor): GeneratedPayload[] {
    ...
    if (field.type === 'string') {
      ...
      if (maxLength !== undefined) {
        payloads.push(
          this.createPayload(field, 'A'.repeat(maxLength), 'boundary-max-exact', 'low',
            `Exact maxLength (${maxLength}) for ${fieldName}`),
          this.createPayload(field, 'A'.repeat(maxLength + 1), 'boundary-overflow', 'high',
            `maxLength + 1 (${maxLength + 1}) overflow for ${fieldName}`),
          this.createPayload(field, 'A'.repeat(maxLength * 10), 'boundary-dos', 'critical',
      ...
      if (minLength !== undefined) {
        payloads.push(
          this.createPayload(field, 'A'.repeat(minLength), 'boundary-min-exact', 'low',
            `Exact minLength (${minLength}) for ${fieldName}`),
          this.createPayload(field, 'A'.repeat(Math.max(0, minLength - 1)), 'boundary-underflow', 'medium',
      ...
      payloads.push(
        this.createPayload(field, '', 'boundary-empty', 'medium',
    ...
    if (field.type === 'number') {
      ...
      if (maximum !== undefined) {
        payloads.push(
          this.createPayload(field, maximum, 'boundary-max-exact', 'low',
            `Exact maximum (${maximum}) for ${fieldName}`),
          this.createPayload(field, maximum + 1, 'boundary-overflow', 'high',
            `maximum + 1 (${maximum + 1}) overflow for ${fieldName}`),
          this.createPayload(field, maximum + 1000, 'boundary-overflow-large', 'high',
      ...
      if (minimum !== undefined) {
        payloads.push(
          this.createPayload(field, minimum, 'boundary-min-exact', 'low',
            `Exact minimum (${minimum}) for ${fieldName}`),
          this.createPayload(field, minimum - 1, 'boundary-underflow', 'high',
            `minimum - 1 (${minimum - 1}) underflow for ${fieldName}`),
          this.createPayload(field, minimum - 1000, 'boundary-underflow-large', 'high',
      ...
      payloads.push(
        this.createPayload(field, 0, 'special-zero', 'low',
          `Zero value for ${fieldName}`),
        this.createPayload(field, -0, 'special-negative-zero', 'low',
          `Negative zero for ${fieldName}`),
        this.createPayload(field, Infinity, 'special-infinity', 'high',
          `Infinity for ${fieldName}`),
        this.createPayload(field, -Infinity, 'special-neg-infinity', 'high',
          `Negative infinity for ${fieldName}`),
        this.createPayload(field, NaN, 'special-nan', 'medium',
          `NaN for ${fieldName}`),
        this.createPayload(field, Number.MAX_SAFE_INTEGER, 'special-max-safe-int', 'medium',
          `MAX_SAFE_INTEGER for ${fieldName}`),
        this.createPayload(field, Number.MIN_SAFE_INTEGER, 'special-min-safe-int', 'medium',
    ...
    if (field.type === 'array') {
      ...
      if (maxItems !== undefined) {
        payloads.push(
          this.createPayload(field, Array(maxItems).fill('item'), 'boundary-max-items', 'low',
            `Exact maxItems (${maxItems}) for ${fieldName}`),
          this.createPayload(field, Array(maxItems + 1).fill('item'), 'boundary-overflow-items', 'high',
            `maxItems + 1 (${maxItems + 1}) for ${fieldName}`),
          this.createPayload(field, Array(maxItems * 100).fill('item'), 'boundary-dos-items', 'critical',
      ...
      if (minItems !== undefined) {
        payloads.push(
          this.createPayload(field, Array(minItems).fill('item'), 'boundary-min-items', 'low',
            `Exact minItems (${minItems}) for ${fieldName}`),
          this.createPayload(field, Array(Math.max(0, minItems - 1)).fill('item'), 'boundary-underflow-items', 'medium',
      ...
      payloads.push(
        this.createPayload(field, [], 'boundary-empty-array', 'medium',
  ...
  private nullAttacks(field: FieldDescriptor): GeneratedPayload[] {
    ...
    return [
      this.createPayload(field, null, 'null-injection', 'medium',
        `Null value for ${fieldName}`),
      this.createPayload(field, undefined, 'undefined-injection', 'medium',
  ...
  private enumAttacks(field: FieldDescriptor): GeneratedPayload[] {
    ...
    payloads.push(
      this.createPayload(field, 'INVALID_ENUM_VALUE', 'enum-invalid', 'critical',
        `Value outside enum for ${fieldName} (privilege escalation test)`)
    );

    // If enum contains string values, test variations
    const stringValues = allowedValues.filter(v => typeof v === 'string') as string[];
    ...
    if (stringValues.length > 0) {
      ...
      if (firstString !== firstString.toUpperCase()) {
        payloads.push(
          this.createPayload(field, firstString.toUpperCase(), 'enum-case-upper', 'high',
            `Uppercase enum value for ${fieldName}`)
        );
      }
      if (firstString !== firstString.toLowerCase()) {
        payloads.push(
          this.createPayload(field, firstString.toLowerCase(), 'enum-case-lower', 'high',
      ...
      payloads.push(
        this.createPayload(field, ` ${firstString}`, 'enum-leading-space', 'high',
          `Leading space in enum for ${fieldName}`),
        this.createPayload(field, `${firstString} `, 'enum-trailing-space', 'high',
          `Trailing space in enum for ${fieldName}`),
        this.createPayload(field, `\t${firstString}`, 'enum-tab', 'medium',
      ...
      const isRoleEnum = stringValues.some(v =>
        ['user', 'admin', 'guest', 'moderator', 'root', 'superuser'].includes(v.toLowerCase())
      );

      if (isRoleEnum) {
        payloads.push(
          this.createPayload(field, 'admin', 'enum-privilege-admin', 'critical',
            `Privilege escalation attempt: admin for ${fieldName}`),
          this.createPayload(field, 'superadmin', 'enum-privilege-superadmin', 'critical',
            `Privilege escalation attempt: superadmin for ${fieldName}`),
          this.createPayload(field, 'root', 'enum-privilege-root', 'critical',
    ...
    payloads.push(
      this.createPayload(field, [allowedValues[0]], 'enum-array', 'high',
        `Array instead of enum value for ${fieldName}`),
      this.createPayload(field, { value: allowedValues[0] }, 'enum-object', 'high',
        `Object instead of enum value for ${fieldName}`),
      this.createPayload(field, allowedValues, 'enum-all-values', 'medium',
  ...
  private formatAttacks(field: FieldDescriptor): GeneratedPayload[] {
    ...
    switch (format) {
      case 'email':
        payloads.push(
          this.createPayload(field, 'invalid', 'format-email-invalid', 'medium',
            `Invalid email for ${fieldName}`),
          this.createPayload(field, '@example.com', 'format-email-no-local', 'medium',
            `Email without local part for ${fieldName}`),
          this.createPayload(field, 'user@', 'format-email-no-domain', 'medium',
            `Email without domain for ${fieldName}`),
          this.createPayload(field, 'user@localhost', 'format-email-localhost', 'low',
            `Email with localhost for ${fieldName}`),
          this.createPayload(field, 'user+tag@example.com', 'format-email-plus', 'low',
            `Email with + character for ${fieldName}`),
          this.createPayload(field, '"()<>[]:,;@\\"!#$%&\'*+-/=?^_`{}|~.a"@example.org', 'format-email-special', 'medium',
            `Email with special characters for ${fieldName}`),
          this.createPayload(field, 'user@' + 'a'.repeat(255) + '.com', 'format-email-long-domain', 'high',
      ...
      case 'url':
        payloads.push(
          this.createPayload(field, 'not a url', 'format-uri-invalid', 'medium',
            `Invalid URI for ${fieldName}`),
          this.createPayload(field, 'javascript:alert(1)', 'format-uri-javascript', 'critical',
            `JavaScript URI (XSS) for ${fieldName}`),
          this.createPayload(field, 'data:text/html,<script>alert(1)</script>', 'format-uri-data-xss', 'critical',
            `Data URI with XSS for ${fieldName}`),
          this.createPayload(field, 'file:///etc/passwd', 'format-uri-file', 'high',
            `File URI (path traversal) for ${fieldName}`),
          this.createPayload(field, 'http://169.254.169.254/latest/meta-data/', 'format-uri-ssrf-aws', 'critical',
            `SSRF attempt (AWS metadata) for ${fieldName}`),
          this.createPayload(field, 'http://localhost:8080/admin', 'format-uri-ssrf-localhost', 'high',
            `SSRF attempt (localhost) for ${fieldName}`),
          this.createPayload(field, 'http://' + 'a'.repeat(2000) + '.com', 'format-uri-long', 'high',
      ...
      case 'date-time':
        payloads.push(
          this.createPayload(field, 'not a date', 'format-date-invalid', 'medium',
            `Invalid date for ${fieldName}`),
          this.createPayload(field, '9999-12-31', 'format-date-far-future', 'low',
            `Far future date for ${fieldName}`),
          this.createPayload(field, '0000-01-01', 'format-date-year-zero', 'medium',
            `Year zero for ${fieldName}`),
          this.createPayload(field, '2024-02-30', 'format-date-invalid-day', 'medium',
            `Invalid day (Feb 30) for ${fieldName}`),
          this.createPayload(field, '2024-13-01', 'format-date-invalid-month', 'medium',
      ...
      case 'ipv4':
        payloads.push(
          this.createPayload(field, '999.999.999.999', 'format-ipv4-overflow', 'medium',
            `Invalid IPv4 (overflow) for ${fieldName}`),
          this.createPayload(field, '127.0.0.1', 'format-ipv4-localhost', 'low',
            `Localhost IPv4 for ${fieldName}`),
          this.createPayload(field, '0.0.0.0', 'format-ipv4-zero', 'medium',
            `0.0.0.0 for ${fieldName}`),
          this.createPayload(field, '169.254.169.254', 'format-ipv4-metadata', 'high',
      ...
      default:
        ...
        payloads.push(
          this.createPayload(field, 'invalid_format', 'format-generic-invalid', 'medium',
  ...
  private generateStructuralAttacks(schema: ParsedSchema): GeneratedPayload[] {
    ...
    if (schema.additionalProperties === false) {
      ...
      (payload as Record<string, unknown>)['constructor'] = { prototype: { isAdmin: true } };
      ...
      payloads.push({
        ...
        expectedVulnerableBehavior: 'Mass assignment or prototype pollution vulnerability',
    ...
    payloads.push({
      ...
      severity: 'high',
      description: 'Deeply nested object (100 levels) to test parser DoS limits',
   ...
   */
  private buildValidPayloadExcept(schema: ParsedSchema, exceptField: string): Record<string, unknown> {
    const payload: Record<string, unknown> = {};
