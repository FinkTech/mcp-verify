...
describe('SchemaConfusionGenerator - Schema-Aware Fuzzing', () => {
  ...
  describe('Basic Payload Generation', () => {
    it('should generate type confusion attacks for string field', () => {
      ...
      const typeConfusionPayloads = payloads.filter(p => p.type === 'type-confusion');
      expect(typeConfusionPayloads.length).toBeGreaterThan(0);

      // Should have boundary attacks
      const boundaryPayloads = payloads.filter(p => p.type.includes('boundary'));
    ...
    it('should generate boundary attacks for number field', () => {
      ...
      const overflowPayloads = payloads.filter(p =>
        p.description.includes('maximum + 1') || p.description.includes('overflow')
      );
      expect(overflowPayloads.length).toBeGreaterThan(0);

      const underflowPayloads = payloads.filter(p =>
        p.description.includes('minimum - 1') || p.description.includes('underflow')
      );
      expect(underflowPayloads.length).toBeGreaterThan(0);

      // Should have special number values (NaN, Infinity)
      const specialPayloads = payloads.filter(p => p.type.includes('special'));
    ...
    it('should generate enum bypass attacks', () => {
      ...
      const invalidEnumPayload = payloads.find(p => p.type === 'enum-invalid');
      ...
      const privEscPayloads = payloads.filter(p => p.type.includes('enum-privilege'));
  ...
  describe('Format-Specific Attacks', () => {
    it('should generate email format attacks', () => {
      ...
      const emailPayloads = payloads.filter(p => p.type.includes('format-email'));
    ...
    it('should generate URI/URL attacks including SSRF', () => {
      ...
      const xssPayloads = payloads.filter(p =>
        p.description.includes('XSS') || p.type.includes('javascript')
      );
  ...
  describe('Structural Attacks', () => {
    it('should generate missing required field attacks', () => {
      ...
      const structuralPayloads = payloads.filter(p => p.type.includes('structural'));
      ...
      const missingRequired = payloads.filter(p => p.type === 'structural-missing-required');
    ...
    it('should generate prototype pollution attacks when additionalProperties=false', () => {
      ...
      const pollutionPayload = payloads.find(p => p.type === 'structural-additional-props');
    ...
    it('should generate deep nesting DoS attack', () => {
      ...
      const nestingPayload = payloads.find(p => p.type === 'structural-deep-nesting');
  ...
  describe('Real-World Scenarios', () => {
    it('should generate comprehensive attacks for e-commerce schema', () => {
      ...
      const productIdAttacks = payloads.filter(p => p.targetParameter === 'productId');
      const quantityAttacks = payloads.filter(p => p.targetParameter === 'quantity');
      const priorityAttacks = payloads.filter(p => p.targetParameter === 'priority');
      const emailAttacks = payloads.filter(p => p.targetParameter === 'email');
