...
function categorizeError(error: Error | unknown): ErrorCategory {
    ...
    // DNS errors
    if (errorCode === 'ENOTFOUND' || errorMessage.includes('not found') || errorMessage.includes('getaddrinfo')) {
        return 'dns';
    }
    ...
    if (errorMessage.includes('json') || errorMessage.includes('parse') || errorMessage.includes('jsonrpc')) {
        return 'protocol';
    }
 ...
 */
function generateTips(category: ErrorCategory, context?: ErrorContext): string[] {
    ...
    switch (category) {
        ...
        case 'dns':
            return [
                ...
                t('tip_verify_dns_generic', { host: target.replace(/^https?:\/\//, '').split(':')[0] }),
                t('tip_use_ip'),
            ];
...
export function formatError(
    ...
    switch (category) {
        case 'connection':
            ...
            message = t('err_conn_msg', { target: context?.target ? ` at ${context.target}` : '' });
            break;

        case 'dns':
            title = t('err_dns_title');
            message = t('err_dns_msg', { target: context?.target ? `: ${context.target}` : '' });
            break;

        case 'timeout':
            title = t('err_timeout_title');
            message = t('err_timeout_msg', { target: context?.target ? ` (${context.target})` : '' });
            break;
        ...
        case 'unknown':
        default:
            title = context?.command ? t('err_cmd_failed', { command: context.command }) : t('err_unknown_title');
