...
export function createLogger(quiet: boolean): CliLogger {
    return {
        ...
        warn: (msg) => { if (!quiet) console.log(chalk.yellow(msg)); },
        error: (msg) => console.error(chalk.red(msg)),
        debug: (msg) => { if (process.env.DEBUG && !quiet) console.log(chalk.gray(msg)); },
        log: (msg) => { if (!quiet) console.log(msg); }
    };
}
 ...
 */
export function createOutputHandlers(quiet: boolean): { log: CliLogger; spinners: SpinnerFactory } {
    return {
