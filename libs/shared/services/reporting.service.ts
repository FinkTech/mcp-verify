...
interface ComparisonAnalysis {
  readonly mostSecure?: { name: string; scores: { security: number } };
  readonly highestQuality?: { name: string; scores: { quality: number } };
  readonly avgSecurity: number;
}
...
export class ReportingService {
  private static htmlGenerator: ReportGenerator | null = null;
  private static sarifGenerator: ReportGenerator | null = null;
  private static markdownGenerator: ReportGenerator | null = null;
  private static textGenerator: ReportGenerator | null = null;
  ...
  static async saveReport(
    ...
    if (effectiveFormats.includes("json")) {
      try {
        ...
        result.errors.push({ format: "json", message: (e as Error).message });
      }
    }
    ...
    if (effectiveFormats.includes("txt")) {
      try {
        ...
        result.errors.push({ format: "txt", message: (e as Error).message });
      }
    }
    ...
    if (payload.kind === "validation") {
      ...
      if (effectiveFormats.includes("html") && this.htmlGenerator) {
        try {
          const content = this.htmlGenerator.generate(
            payload.data,
            language,
            baselineComparison,
          );
          ...
          result.errors.push({ format: "html", message: (e as Error).message });
        }
      }
  ...
  private static async renderMarkdown(
    ...
    if (payload.kind === "compare") {
      return this.renderComparisonMarkdown(
  ...
  private static async renderText(
    ...
    if (payload.kind === "doctor") {
      ...
      return this.renderDoctorMarkdown(payload.data).replace(
        /#|\||:-+:|`|/g,
        "",
      );
    }
  ...
  private static renderComparisonMarkdown(
    results: ReadonlyArray<ComparisonResult>,
    analysis: ComparisonAnalysis,
    lang: Language,
  ): string {
