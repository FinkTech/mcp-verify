...
export class DenoSandbox implements ISandbox {
  ...
  static checkEnvironment(forceRecheck = false): SandboxEnvironmentCheck {
    ...
    try {
      ...
      const whichCmd =
        process.platform === "win32" ? "where deno" : "which deno";
      ...
      result.issues.push(
        t("sandbox_deno_not_found") || "Deno binary not found in PATH",
      );
      result.suggestions.push(
        t("sandbox_install_deno") ||
          "Install Deno: curl -fsSL https://deno.land/install.sh | sh",
      );
      result.suggestions.push(
        t("sandbox_alt_docker") ||
    ...
    try {
      ...
      if (semver) {
        ...
        if (comparison >= 0) {
          ...
          result.issues.push(
            (
              t("sandbox_deno_version_too_old") ||
              "Deno version {current} is below minimum {required}"
            )
              .replace(
                "{current}",
              ...
              .replace(
                "{required}",
                `${MIN_DENO_VERSION.major}.${MIN_DENO_VERSION.minor}.${MIN_DENO_VERSION.patch}`,
              ),
          );
          result.suggestions.push(
            t("sandbox_update_deno") || "Update Deno: deno upgrade",
          );
        }
      } else {
        result.issues.push(
          t("sandbox_version_parse_failed") || "Could not parse Deno version",
        );
      }
    } catch (error) {
      result.issues.push(
        (
          t("sandbox_version_check_failed") ||
          "Failed to check Deno version: {error}"
        ).replace(
          "{error}",
    ...
    try {
      ...
      const testFile = path.join(
        tempDir,
        `.mcp-verify-sandbox-test-${Date.now()}`,
      );
      ...
      result.suggestions.push(
        t("sandbox_check_temp_perms") ||
          "Check permissions on your temp directory",
      );
    }

    // 4. Determine overall availability
    result.available =
      result.binaryPath !== undefined &&
  ...
  static getStatusMessage(check?: SandboxEnvironmentCheck): string {
    ...
    if (result.available) {
      return (
        t("sandbox_ready") || "✅ Deno sandbox ready (v{version})"
      ).replace(
        "{version}",
        result.semver
...
${t("sandbox_future_version") || "Docker sandbox support for other runtimes coming soon."}`,
    ...
    if (this.options.allowRead && this.options.allowRead.length > 0) {
      const paths = this.options.allowRead
