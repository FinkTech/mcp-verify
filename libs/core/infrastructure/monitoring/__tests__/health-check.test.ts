...
describe('HealthMonitor', () => {
  ...
  describe('Health Checks', () => {
    ...
    it('should include security health check by default', async () => {
      ...
      const securityCheck = report.checks.find(c => c.component === 'Security');
    ...
    it('should report overall HEALTHY status when all checks pass', async () => {
      ...
      expect([HealthStatus.HEALTHY, HealthStatus.DEGRADED]).toContain(report.status);
    });
  ...
  describe('Memory Metrics', () => {
    ...
    it('should calculate memory percentage correctly', () => {
      ...
      expect(Math.abs(metrics.memoryUsage.percentage - expectedPercentage)).toBeLessThan(1);
    });
  });
