...
export class ScanHistoryManager {
  ...
  private validateScanId(scanId: string): void {
    ...
    if (!validPattern.test(scanId)) {
      throw new Error(`Invalid scan ID: ${scanId}. Only alphanumeric characters, underscores, and hyphens are allowed.`);
    }
  }
   ...
   */
  async saveScan(report: Report, options: { baseline?: boolean; version?: string } = {}): Promise<ScanHistory> {
    ...
    const scanHistory: ScanHistory = {
      ...
      protocol_score: report.protocolCompliance ? report.protocolCompliance.score : 100,
  ...
  async loadScan(scanId: string): Promise<ScanHistory | null> {
    ...
    try {
      ...
      throw new Error(t('failed_parse_scan_file', { id: scanId, error: (error as Error).message }));
    }
  }
   ...
   */
  async listScans(options: { serverName?: string; limit?: number } = {}): Promise<ScanHistory[]> {
    ...
    for (const file of files) {
      try {
        ...
        console.warn(t('failed_parse_scan_file', { id: file, error: (error as Error).message }));
        continue;
      }
    }
