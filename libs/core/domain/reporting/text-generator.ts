...
export const KeyValueRow = (
  ...
  value: string,
  indent = "  ",
): string => `${indent}${pad(label + ":", LABEL_WIDTH)} ${value}`;
...
export const Table = (options: TableOptions): string => {
  ...
  if (title) {
    const totalWidth =
      ...
      4;
    lines.push(
      "  │" + pad(centre(title, totalWidth - 2), totalWidth - 2) + "│",
    );
    lines.push(tableSep("═"));
  }
...
export const FindingCard = (data: FindingCardData): string => {
  ...
  if (data.payloads.length > 0) {
    ...
    if (data.payloads.length > MAX_SHOWN) {
      lines.push(
        `${indent}  ... and ${data.payloads.length - MAX_SHOWN} more unique variants`,
      );
    }
  }
...
export const calculateRiskLabel = (
  score: number,
...
): Map<Severity, Map<string, GroupedFinding>> => {
...
export const calculateFuzzPassRate = (fuzzing: FuzzingReport): string => {
  ...
  const rate =
    ((fuzzing.totalTests - fuzzing.failedTests) / fuzzing.totalTests) * 100;
...
const ExecutiveSummarySection = (
  ...
  const summaryTable = Table({
    ...
    title:
      (t as Record<string, string>).executive_summary ?? "EXECUTIVE SUMMARY",
  });
...
const TargetInfoSection = (
  ...
  lines.push(
    SectionHeader(
      (t as Record<string, string>).section_target ?? "TARGET INFO",
    ),
  );
  lines.push("");
  lines.push(
    KeyValueRow(
      (t as Record<string, string>).label_server ?? "Server",
      report.server_name,
    ),
  );
  lines.push(
    KeyValueRow((t as Record<string, string>).label_url ?? "URL", report.url),
  );
  lines.push(
    KeyValueRow(
      (t as Record<string, string>).label_protocol ?? "Protocol",
  ...
  if (report.gitInfo) {
    lines.push(
      KeyValueRow(
        (t as Record<string, string>).label_branch ?? "Branch",
        report.gitInfo.branch,
      ),
    );
    lines.push(
      KeyValueRow(
        (t as Record<string, string>).label_commit ?? "Commit",
        report.gitInfo.revisionId,
      ),
    );
  }
...
const SecurityFindingsSection = (
  ...
  lines.push(
    SectionHeader(
      (t as Record<string, string>).section_security ?? "SECURITY FINDINGS",
    ),
  );
  lines.push("");
  lines.push(
  ...
  if (findings.length === 0) {
    lines.push(
      `  ${(t as Record<string, string>).no_findings ?? "No security findings detected."}`,
    );
    lines.push("");
    return lines.join("\n");
  }
  ...
  for (const sev of SEVERITY_ORDER) {
    ...
    const groupSize = Array.from(severityGroup.values()).reduce(
      (sum, g) => sum + g.count,
      0,
    );
    lines.push(
      `  ── ${sevLabel} (${groupSize}) ${"─".repeat(Math.max(0, PAGE_WIDTH - 12 - sevLabel.length - String(groupSize).length))}`,
    );
    lines.push("");
    ...
    for (const entry of severityGroup.values()) {
      ...
      const cardData: FindingCardData = {
        ...
        labels: {
          ...
          component:
            (t as Record<string, string>).label_component ?? "Component",
          cwe: (t as Record<string, string>).label_cwe ?? "CWE",
          remediation:
            (t as Record<string, string>).label_remediation ?? "Remediation",
        },
      };
...
const FuzzingSection = (
  ...
  lines.push(
    SectionHeader(
      (t as Record<string, string>).section_fuzzing ?? "SMART FUZZING ANALYSIS",
    ),
  );
  lines.push("");
  ...
  if (!fuzzing.executed) {
    lines.push(
      `  ${(t as Record<string, string>).fuzzing_not_executed ?? "Fuzzing was not executed in this session."}`,
    );
    lines.push("");
    return lines.join("\n");
  }
  ...
  lines.push(
    KeyValueRow(
      (t as Record<string, string>).label_total_tests ?? "Total Tests",
      String(fuzzing.totalTests),
    ),
  );
  lines.push(
    KeyValueRow(
      (t as Record<string, string>).label_failed_tests ?? "Failed Tests",
      `${fuzzing.failedTests} (${failRate}%)`,
    ),
  );
  lines.push(
    KeyValueRow(
      (t as Record<string, string>).label_pass_rate ?? "Pass Rate",
      `${passRate}%`,
    ),
  );
  lines.push(
    KeyValueRow(
      (t as Record<string, string>).label_crashes ?? "Crashes",
      String(fuzzing.crashes),
    ),
  );
...
const CapabilitiesSection = (
  ...
  lines.push(
    SectionHeader(
      (t as Record<string, string>).section_capabilities ??
  ...
  if (report.tools?.items && report.tools.items.length > 0) {
    ...
    lines.push("");
    lines.push(
      `  ${(t as Record<string, string>).label_tool_list ?? "Tool list"}:`,
    );
    ...
    if (report.tools.items.length > MAX_SHOWN) {
      lines.push(
        `    … ${report.tools.items.length - MAX_SHOWN} ${(t as Record<string, string>).label_more ?? "more"}`,
      );
    }
  }
...
const FooterSection = (t: (typeof translations)["en"]): string => {
  const msg =
    (t as Record<string, string>).footer_generated_by ??
