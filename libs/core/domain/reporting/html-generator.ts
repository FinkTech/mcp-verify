...
export class HtmlReportGenerator {
  static generate(
    ...
    const protocolPassed = report.protocolCompliance
      ? report.protocolCompliance.passed
      : true;
    ...
    const findingsByS = report.security.findings.reduce(
      (acc, f) => {
    ...
    const findingCardsHtml = sortedFindings
      .map((finding, idx) => {
        ...
        const recommendation =
          finding.remediation || "Review and remediate this security issue.";
        const recForJs = escapeForJs(recommendation);

        // Detect if finding has fuzzer payload
        const hasFuzzerPayload =
          finding.evidence &&
          typeof finding.evidence === "string" &&
          (finding.evidence as string).includes("mcp-verify fuzz");

        // Detect if finding is validation issue (SEC-019)
        const isValidationIssue =
          code === "SEC-019" || finding.message?.includes("validation");
    ...
    <div class="glass-panel-depth p-0 overflow-hidden border-glow-hover flex flex-col h-full">
        ...
        <div class="p-4 space-y-2 flex-1 bg-gradient-to-b from-critical/[0.02] to-transparent">
            ...
            `,
                    ...
                    .join("")
                : `<p class="text-sm text-slate-500 text-center py-8">${t.no_new_risks || "No new risks introduced"}</p>`
            }
        </div>
    </div>
    <div class="glass-panel-depth p-0 overflow-hidden border-glow-hover flex flex-col h-full">
        ...
        <div class="p-4 space-y-2 flex-1 bg-gradient-to-b from-emerald-500/[0.02] to-transparent">
            ...
            `,
                    ...
                    .join("")
                : `<p class="text-sm text-slate-500 text-center py-8">${t.no_risks_resolved || "No risks resolved"}</p>`
            }
        </div>
    </div>
</div>
...
<body class="bg-background-dark text-slate-200 font-display antialiased min-h-screen relative overflow-x-hidden selection:bg-violet-500/30 selection:text-white pb-40">
    ...
    <main class="relative z-10 w-full max-w-[1000px] mx-auto px-6 py-12 flex flex-col gap-8">
        ...
        ${(() => {
          ...
          if (hasCritical) {
            ratingLevel =
              t.requires_immediate_action || "Requires Immediate Action";
            ratingColor = "critical";
            ratingIcon = "emergency";
            postureText =
              t.critical_findings_detected || "Critical findings detected";
          } else if (hasHigh || score < 70) {
            ratingLevel = t.requires_attention || "Requires Attention";
            ratingColor = "high";
            ratingIcon = "priority_high";
            postureText =
              t.moderate_findings_detected || "Moderate findings detected";
          } else if (score >= 90) {
            ...
            postureText =
              t.minor_findings_detected || "Minor findings detected";
          }
                    ...
                    <div class="space-y-2">
                        ${
                          topIssues.length > 0
                            ? topIssues
                                .map((issue, idx) => {
                                  const sevColor = getSeverityTailwindColor(
                                    issue.severity || "low",
                                  );
                                  const message =
            ...
            <div class="space-y-3">
                ${sortedFindings
                  .map((finding, idx) => {
                    ...
                    const sevColor = getSeverityTailwindColor(sev);
                    const code =
                      finding.rule || `SEC-${String(idx + 1).padStart(3, "0")}`;
                    const message =
                      finding.message || "Security issue detected";

                    // Estimate time to fix based on severity
                    let timeEstimate = "";
                    if (sev === "critical")
                      timeEstimate = `4 ${t.hours || "hours"}`;
                    else if (sev === "high")
                      timeEstimate = `2 ${t.hours || "hours"}`;
                    else if (sev === "medium")
                      timeEstimate = `1 ${t.hour || "hour"}`;
...
function getSeverityBgColor(severity: string) {
  if (severity === "critical")
    return "bg-critical/10 text-critical border border-critical/20 shadow-[0_0_10px_rgba(239,68,68,0.15)]";
  if (severity === "high")
    return "bg-high/10 text-high border border-high/20 shadow-[0_0_10px_rgba(249,115,22,0.2)]";
  if (severity === "medium")
    return "bg-medium/10 text-medium border border-medium/20";
...
function generateSurgicalSummary(
  report: Report,
  t: (typeof translations)["en"],
): string {
  const hasFindings = report.security.findings.length > 0;
  const hasCritical = report.security.findings.some(
...
function generateHeatmapData(findings: SecurityFinding[]): number[][] {
  ...
  findings.forEach((f) => {
    ...
    if (sev === "critical") {
      impact = 2;
      prob = 2; // High impact, high probability
    } else if (sev === "high") {
      impact = 2;
      prob = 1; // High impact, medium probability
    } else if (sev === "medium") {
      impact = 1;
      prob = 1; // Medium impact, medium probability
    } else {
      impact = 0;
      prob = 1; // Low impact, medium probability
    }
...
function generateHeatmapCells(heatmapData: number[][]): string {
  ...
  for (let row = 2; row >= 0; row--) {
    for (let col = 0; col <= 2; col++) {
      ...
      const cellClass =
        "heatmap-cell rounded-lg flex items-center justify-center group relative cursor-pointer";
      let bgClass = "";
      let textClass = "";
      let extraClass = "";
      ...
      if (severity === "critical") {
        bgClass =
          "bg-gradient-to-br from-critical/30 to-critical/10 border border-critical/30 hover:border-critical/50";
        textClass = "text-white font-mono font-bold text-2xl drop-shadow-md";
        extraClass =
          "shadow-[0_0_15px_rgba(239,68,68,0.15)] hover:shadow-[0_0_25px_rgba(239,68,68,0.3)] ring-1 ring-critical/20";
        if (count > 0) {
          extraClass += " relative";
        }
      } else if (severity === "high") {
        bgClass =
          "bg-gradient-to-br from-high/20 to-high/5 border border-high/20 hover:border-high/40";
        textClass = `text-high/80 font-mono font-bold text-xl group-hover:text-high transition-colors`;
      } else if (severity === "medium") {
        bgClass =
          "bg-gradient-to-br from-medium/10 to-medium/5 border border-medium/10 hover:border-medium/30";
...
function generateComplianceMatrix(findings: SecurityFinding[]): {
  owasp: number;
  cwe: number;
  nist: number;
  pci: number;
  ...
  const ruleMapping: Record<
    string,
    { owasp?: string; cwe?: string; nist?: string; pci?: string; iso?: string }
  > = {
    "SEC-001": {
      owasp: "A07:2021",
      cwe: "CWE-287",
...
function generateAgencyTags(
  ...
  if (
    ...
    tags.push({
      icon: "lan",
      label: t.agency_network || "Network",
      color: "violet",
    });
  }
  if (
