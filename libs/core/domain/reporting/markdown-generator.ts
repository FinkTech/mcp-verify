...
export class MarkdownReportGenerator {
  static generate(report: Report, lang: Language = 'en'): string {
    ...
    const now = new Date(report.timestamp).toLocaleString(lang === 'es' ? 'es-ES' : 'en-US');
    ...
    if (report.security.findings.length > 0) {
      ...
      report.security.findings.forEach(finding => {
        ...
        const severityLabel = t[finding.severity as keyof typeof t] || finding.severity;
    ...
    if (report.tools.items && report.tools.items.length > 0) {
        ...
        markdown += `|------|-------|-------------|\n`;
        report.tools.items.forEach(tool => {
             const desc = tool.description ? tool.description.slice(0, 50) + (tool.description.length > 50 ? '...' : '') : 'N/A';
  ...
  private static getRulePdfUrl(ruleCode: string, lang: Language = 'en'): string {
  ...
  private static getBadgeColor(score: number | string, type: 'security' | 'quality'): string {
    if (type === 'security') {
