...
import { Report, SecurityFinding } from '../../mcp-server/entities/validation.types';
...
describe('SarifGenerator', () => {
  ...
  describe('generate()', () => {
    it('should generate valid SARIF 2.1.0 structure', () => {
      ...
      expect(sarif).toHaveProperty('$schema', 'https://json.schemastore.org/sarif-2.1.0.json');
    ...
    it('should include tool driver metadata', () => {
      ...
      expect(driver).toHaveProperty('informationUri', 'https://github.com/FinkTech/mcp-verify');
    ...
    it('should map security findings to SARIF results', () => {
      ...
      expect(criticalResult.message).toHaveProperty('text', 'SQL injection pattern detected');
      expect(criticalResult.properties).toHaveProperty('evidence');
      expect(criticalResult.properties.evidence).toEqual({ description: 'Parameter accepts raw SQL' });
      expect(criticalResult.properties).toHaveProperty('remediation', 'Use parameterized queries');
    });
