  ...
  static generateMermaid(
    ...
    if (discovery.resources && discovery.resources.length > 0) {
      ...
      discovery.resources.forEach((resource, i) => {
        const id = `R${i}`;
        const safeName = (resource.name || "unknown").replace(
          /[^a-zA-Z0-9]/g,
          "_",
        );
    ...
    if (discovery.prompts && discovery.prompts.length > 0) {
      ...
      discovery.prompts.forEach((prompt, i) => {
        const id = `P${i}`;
        const safeName = (prompt.name || "unknown").replace(
          /[^a-zA-Z0-9]/g,
          "_",
        );
    ...
    if (theme === "light") {
      lines.push(
        "    classDef server fill:#f1f5f9,stroke:#3b82f6,stroke-width:2px,color:#1e293b",
      );
      lines.push(
        "    classDef tool fill:#e2e8f0,stroke:#22c55e,stroke-width:1px,color:#1e293b",
      );
      lines.push(
        "    classDef resource fill:#e2e8f0,stroke:#eab308,stroke-width:1px,color:#1e293b",
      );
      lines.push(
        "    classDef prompt fill:#e2e8f0,stroke:#a855f7,stroke-width:1px,color:#1e293b",
      );
    } else {
      lines.push(
      ...
      lines.push(
        "    classDef tool fill:#1e293b,stroke:#22c55e,stroke-width:1px,color:#fff",
      );
      lines.push(
        "    classDef resource fill:#1e293b,stroke:#eab308,stroke-width:1px,color:#fff",
      );
      lines.push(
        "    classDef prompt fill:#1e293b,stroke:#a855f7,stroke-width:1px,color:#fff",
      );
    }
