...
import type { Report, SecurityFinding, FuzzingReport, GitInfo } from '../mcp-server/entities/validation.types';
import { Logger } from '../../infrastructure/logging/logger';
import type { AuditEntry } from '../../infrastructure/logging/logger';
import type { SystemMetrics, SystemHealthReport } from '../../infrastructure/monitoring/health-check';
...
export interface EnhancedSecurityReport {
  ...
  owaspTop10Coverage: {
    total: number; covered: number; coverage: string; mappings: Record<string, number>;
  };
  cweCategories: { category: string; count: number; severity: string }[];
  prioritizedRemediation: {
    priority: number; finding: SecurityFinding;
    effort: 'Low' | 'Medium' | 'High'; impact: 'Low' | 'Medium' | 'High';
  }[];
  guardrailsReport?: {
    totalInterventions: number; blocked: number; modified: number;
    piiRedacted: number; rateLimitHits: number;
  };
}
...
export interface AuditReport {
  ...
  complianceStatus: {
    auditTrailComplete: boolean; retentionCompliant: boolean; encryptionEnabled: boolean;
  };
}
    ...
    tool: {
      driver: {
        ...
        rules: Array<{
          ...
          shortDescription: { text: string }; fullDescription: { text: string };
          defaultConfiguration: { level: string }; helpUri?: string;
        }>;
      };
    };
    results: Array<{
      ruleId: string; level: 'error' | 'warning' | 'note';
      message: { text: string };
      locations: Array<{
        physicalLocation?: {
...
export class EnhancedReporter {
  ...
  generateEnhancedReport(
    ...
    const enhancedFuzzing  = baseReport.fuzzing ? this.enhanceFuzzingReport(baseReport.fuzzing) : undefined;
    const performance      = this.generatePerformanceReport(baseReport.duration_ms, metrics);
    const audit            = this.generateAuditReport(auditTrail);
    const executiveSummary = this.generateExecutiveSummary(enhancedSecurity, enhancedFuzzing, performance, audit);
    ...
    const report: EnhancedReport = {
      ...
      performance, audit, health, executiveSummary,
    ...
    try {
      ...
      this.logger.warn('attestation: token generation failed — report unsigned', {
  ...
  public verifyReport(report: EnhancedReport): VerificationResult {
    ...
    if (!tokenStr) {
      return { valid: false, error: 'metadata.signature missing', diagnostics: diag };
    }
    ...
    const machineKey   = this.deriveMachineKey(masterSecret, attestation.fingerprintHash);
    ...
    const proofMatch = this.timingSafeHexEqual(token.machineKeyProof, expectedProof);
    ...
    // ── Phase 2: Verify HMAC ───────────────────────────────────────────────
    const signedMessage  = this.buildSignedMessage(token.nonce, reportDigest, attestationHash);
    ...
    const attestationMatch       = this.timingSafeHexEqual(currentAttestationHash, attestationHash);
   ...
   */
  private deriveMachineKey(masterSecret: string, fingerprintHash: string): Buffer {
    return Buffer.from(
      crypto
  ...
  private buildIntegrityToken(report: EnhancedReport): string {
    const attestation = report.metadata?.attestation;
    if (!attestation) {
      throw new Error('buildIntegrityToken: metadata.attestation debe estar presente antes de firmar');
    }
    ...
    const signedMessage = this.buildSignedMessage(nonce, reportDigest, attestationHash);
    const hmac = crypto
      .createHmac('sha256', machineKey)
      .update(signedMessage, 'utf8')
      .digest('hex');
  ...
  private buildSignedMessage(nonce: string, reportDigest: string, attestationHash: string): string {
  ...
  private parseToken(raw: string): ParsedToken {
    ...
    if (parts.length !== 4) {
      throw new Error(`Se esperaban 4 partes delimitadas por ':', se recibieron ${parts.length}`);
    }
    const [version, machineKeyProof, nonce, hmac] = parts;

    if (version !== this.TOKEN_VERSION) {
      throw new Error(`Versión desconocida "${version}". Esperada: "${this.TOKEN_VERSION}"`);
    }
  ...
  private getEnvironmentFingerprint(): OriginAttestation {
    ...
    if (trustLevel !== 'TRUSTED') {
      this.logger.warn(`attestation: entorno ${trustLevel}`, { degradedSources });
    }

    return {
      hostname, platform, arch,
      networkFingerprint, fingerprintHash,
      gitCommit, timestamp, trustLevel, degradedSources,
    };
  }
  ...
  private _jcs(value: unknown): string {
    ...
    if (typeof value === 'number') {
      if (!Number.isFinite(value)) {
        this.logger.warn('jcs: número no-finito serializado como null', { value });
        return 'null';
      }
  ...
  private _jcsString(s: string): string {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      else if (cp === 0x5c) { out += '\\\\'; continue; }
      else if (cp === 0x08) { out += '\\b';  continue; }
      else if (cp === 0x09) { out += '\\t';  continue; }
      else if (cp === 0x0a) { out += '\\n';  continue; }
      else if (cp === 0x0c) { out += '\\f';  continue; }
      else if (cp === 0x0d) { out += '\\r';  continue; }
      else if (cp < 0x20)   { out += '\\u' + cp.toString(16).padStart(4, '0'); continue; }
      out += s[i];
    }
    return out + '"';
  }
  ...
  private classifyTrustLevel(degraded: string[]): AttestationTrustLevel {
    const hardwareSources  = new Set(['hostname', 'platform', 'networkFingerprint']);
    const hardwareDegraded = degraded.filter(s => hardwareSources.has(s)).length;
  ...
  private enhanceSecurityReport(base: Report['security']): EnhancedSecurityReport {
    const findings      = base.findings || [];
    const criticalCount = findings.filter((f: SecurityFinding) => f.severity === 'critical').length;
    const highCount     = findings.filter((f: SecurityFinding) => f.severity === 'high').length;
    const mediumCount   = findings.filter((f: SecurityFinding) => f.severity === 'medium').length;
    const lowCount      = findings.filter((f: SecurityFinding) => f.severity === 'low').length;
    const riskScore     = Math.min(100, criticalCount * 25 + highCount * 10 + mediumCount * 5 + lowCount);

    const businessImpact = (
      criticalCount > 0 ? 'Critical' : highCount > 2 ? 'High' : highCount > 0 ? 'Medium' : 'Low'
    ) as EnhancedSecurityReport['businessImpact'];

    const exploitability = (
      criticalCount > 0 || highCount > 1 ? 'High' : highCount > 0 || mediumCount > 5 ? 'Medium' : 'Low'
    ) as EnhancedSecurityReport['exploitability'];

    return {
      score: base.score, level: this.determineRiskLevel(riskScore),
      grade: this.calculateGrade(riskScore), findings,
      criticalCount, highCount, mediumCount, lowCount,
      riskScore, businessImpact, exploitability,
  ...
  private enhanceFuzzingReport(base: FuzzingReport): EnhancedFuzzingReport {
    ...
    const securityPayloadCount = results.filter(r => r.payloadType.includes('Security')).length;
    const mutationCount        = results.filter(r => r.payloadType.includes('Mutated')).length;
    const vulnerabilityCount   = base.vulnerabilities?.length ?? 0;
    const avgResponseTime      = results.length > 0
      ? results.reduce((s, r) => s + r.durationMs, 0) / results.length : 0;
    const slowest = results.reduce(
      (max, r) => r.durationMs > max.durationMs ? r : max,
      results[0] ?? { durationMs: 0, payloadType: '' }
    );
    return {
      ...base, securityPayloadCount, mutationCount, vulnerabilityCount,
      avgResponseTime: Math.round(avgResponseTime),
      slowestPayload: slowest.durationMs > 0
        ? { type: slowest.payloadType, responseTime: slowest.durationMs } : undefined,
      vulnerabilitySummary: this.generateVulnerabilitySummary(base.vulnerabilities ?? []),
    };
  }

  private generatePerformanceReport(executionTime: number, metrics: SystemMetrics): PerformanceReport {
    return {
      executionTime,
      avgResponseTime: metrics.avgResponseTime,
      p95ResponseTime: metrics.p95ResponseTime,
      p99ResponseTime: metrics.p99ResponseTime,
      memoryUsage:  { peak: metrics.memoryUsage.used, average: metrics.memoryUsage.used, current: metrics.memoryUsage.used },
      throughput:   { requestsPerSecond: metrics.requestsPerSecond, validationsPerSecond: metrics.requestsPerSecond },
      efficiency:   { score: this.calculateEfficiencyScore(metrics), bottlenecks: this.identifyBottlenecks(metrics) },
    };
  }

  private generateAuditReport(auditTrail: AuditEntry[]): AuditReport {
    const criticalEvents = auditTrail.filter(e => e.severity === 'critical').length;
    const securityEvents = auditTrail.filter(e =>
      e.eventType.includes('security') || e.eventType.includes('guardrail')
    ).length;
    const eventsByType: Record<string, number> = {};
    for (const e of auditTrail) eventsByType[e.eventType] = (eventsByType[e.eventType] ?? 0) + 1;
    return {
      totalEvents: auditTrail.length, criticalEvents, securityEvents, eventsByType,
      timeline: auditTrail.slice(-20).map(e => ({ timestamp: e.timestamp, event: e.eventType, severity: e.severity })),
      complianceStatus: { auditTrailComplete: true, retentionCompliant: true, encryptionEnabled: false },
    };
  }

  private generateExecutiveSummary(
    security: EnhancedSecurityReport, fuzzing?: EnhancedFuzzingReport,
    performance?: PerformanceReport, _audit?: AuditReport
  ): EnhancedReport['executiveSummary'] {
    const keyFindings: string[] = [];
    const criticalActions: string[] = [];
    if (security.criticalCount > 0) {
      keyFindings.push(`${security.criticalCount} critical security issue(s) detected`);
      criticalActions.push('Immediately address critical security vulnerabilities');
    }
    ...
    if (fuzzing && fuzzing.vulnerabilityCount > 0)
      keyFindings.push(`${fuzzing.vulnerabilityCount} potential vulnerabilities discovered via fuzzing`);
    ...
    return {
      ...
      keyFindings, criticalActions, passedChecks, totalChecks,
      complianceScore: Math.round((passedChecks / totalChecks) * 100),
    };
  }

  private calculateGrade(s: number): 'A+' | 'A' | 'B' | 'C' | 'D' | 'F' {
    return s === 0 ? 'A+' : s < 10 ? 'A' : s < 25 ? 'B' : s < 50 ? 'C' : s < 75 ? 'D' : 'F';
  }
  private determineRiskLevel(s: number): 'Low Risk' | 'Medium Risk' | 'High Risk' | 'Critical Risk' {
    return s < 10 ? 'Low Risk' : s < 25 ? 'Medium Risk' : s < 50 ? 'High Risk' : 'Critical Risk';
  }
  private calculateOWASPCoverage(findings: SecurityFinding[]): EnhancedSecurityReport['owaspTop10Coverage'] {
    const mappings: Record<string, number> = {};
    for (const f of findings) if (f.ruleCode) mappings[f.ruleCode] = (mappings[f.ruleCode] ?? 0) + 1;
    const covered = Object.keys(mappings).length;
    return { total: 10, covered, coverage: `${covered}/10`, mappings };
  }
  private extractCWECategories(findings: SecurityFinding[]): EnhancedSecurityReport['cweCategories'] {
    const cats: Record<string, { count: number; severity: string }> = {};
    for (const f of findings) {
      if (f.ruleCode) {
        if (!cats[f.ruleCode]) cats[f.ruleCode] = { count: 0, severity: f.severity };
        cats[f.ruleCode].count++;
      }
    }
    return Object.entries(cats).map(([category, d]) => ({ category, count: d.count, severity: d.severity }));
  }
  private prioritizeRemediation(findings: SecurityFinding[]): EnhancedSecurityReport['prioritizedRemediation'] {
    const w: Record<string, number> = { critical: 4, high: 3, medium: 2, low: 1, info: 0 };
    return findings
      .map(f => ({ priority: w[f.severity] ?? 0, finding: f, effort: 'Medium' as const, impact: this.estimateImpact(f) }))
      .sort((a, b) => b.priority - a.priority)
      .slice(0, 10);
  }
  private estimateImpact(f: SecurityFinding): 'Low' | 'Medium' | 'High' {
    return f.severity === 'critical' || f.severity === 'high' ? 'High' : 'Medium';
  }
  private generateVulnerabilitySummary(
    vulnerabilities: Array<{
      toolName: string; payloadType: string;
      findings: Array<{ type: string; severity: 'critical'|'high'|'medium'|'low'; description: string; evidence: string; remediation: string }>;
    }>
    ...
    for (const v of vulnerabilities) {
      ...
      for (const f of v.findings ?? []) bySeverity[f.severity] = (bySeverity[f.severity] ?? 0) + 1;
    }
    return {
      byType, bySeverity,
  ...
  private calculateEfficiencyScore(m: SystemMetrics): number {
    const rt  = m.avgResponseTime < 100 ? 100 : m.avgResponseTime < 500 ? 80 : m.avgResponseTime < 1000 ? 60 : 40;
    const mem = m.memoryUsage.percentage < 70 ? 100 : m.memoryUsage.percentage < 85 ? 80 : 60;
  ...
  private calculateOverallRisk(s: EnhancedSecurityReport, f?: EnhancedFuzzingReport): 'Low' | 'Medium' | 'High' | 'Critical' {
    if (s.criticalCount > 0 || (f && f.vulnerabilityCount > 5)) return 'Critical';
    if (s.highCount > 2     || (f && f.vulnerabilityCount > 2)) return 'High';
    if (s.highCount > 0     || s.mediumCount > 5)               return 'Medium';
    return 'Low';
  }
  private extractRulesFromFindings(findings: SecurityFinding[]): SARIFReport['runs'][0]['tool']['driver']['rules'] {
    const map = new Map<string, SARIFReport['runs'][0]['tool']['driver']['rules'][0]>();
    for (const f of findings) {
      if (f.ruleCode && !map.has(f.ruleCode)) {
        map.set(f.ruleCode, {
          id: f.ruleCode, name: f.component,
          shortDescription: { text: f.message }, fullDescription: { text: f.message },
  ...
  private convertFindingsToSARIFResults(findings: SecurityFinding[]): SARIFReport['runs'][0]['results'] {
    return findings.map(f => ({
      ruleId: f.ruleCode ?? 'UNKNOWN',
      level:  (f.severity === 'critical' || f.severity === 'high' ? 'error' : 'warning') as 'error' | 'warning' | 'note',
      message: { text: f.message },
      locations: [{ logicalLocations: [{ name: f.component, kind: 'component' }] }],
    }));
  }
}
