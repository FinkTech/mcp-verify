...
describe('InsecureDeserializationRule', () => {
    ...
    describe('should detect vulnerabilities', () => {
        it('should detect dangerous formats (Critical)', () => {
            ...
            expect(critical!.message).toContain('appears to deserialize dangerous format:');
        ...
        it('should detect unsafe YAML loading (Critical)', () => {
            ...
            const findings = rule.evaluate(discovery);
            expect(findings.length).toBeGreaterThan(0);
            const yamlFinding = findings.find(f => f.message.includes('unsafe YAML'));
        ...
        it('should detect untyped parameters (Critical)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const paramFinding = findings.find(f => f.location?.parameter === 'payload');
            expect(paramFinding).toBeDefined();
            expect(paramFinding!.severity).toBe('critical');
            expect(paramFinding!.message).toContain('is an object without explicit type, allowing arbitrary object injection');
        });
        ...
        it('should detect arbitrary object parameters (High)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const paramFinding = findings.find(f => f.location?.parameter === 'data');
            expect(paramFinding).toBeDefined();
            expect(paramFinding!.severity).toBe('high');
            expect(paramFinding!.message).toContain('allows arbitrary object properties, which can lead to injection attacks');
        });
