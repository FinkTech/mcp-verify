...
const WEAK_ENCRYPTION: ReadonlyArray<EncryptionEntry> = [
  ...
  {
    name: "3DES",
    pattern: /\b(?:3DES|TDES|Triple[-_]?DES)\b/i,
    severity: "critical",
  },
...
const CRYPTO_CONTEXT =
  /\b(?:encrypt(?:s|ed|ing|ion)?|decrypt(?:s|ed|ing|ion)?|cipher|hash(?:es|ed|ing)?|sign(?:s|ed|ing)?|verify|verif(?:ies|ied|ying)|digest|crypt(?:o(?:graph(?:y|ic|ically))?)?|hmac|mac|token)/i;
const KEY_SIZE_PARAM =
  /^(?:key(?:_?size|_?len(?:gth)?|bits)|bits|key[-_]?length)$/i;
const ALGO_PARAM =
  /^(?:algorithm|algo|cipher|hash[-_]?type|enc(?:oding|ryption)?[-_]?(?:type|algo(?:rithm)?))$/i;
...
export class WeakCryptographyRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_weak_crypto_name");
  }
  get description() {
    return t("sec_weak_crypto_desc");
  }
  readonly helpUri =
    ...
    for (const { name, pattern, severity } of WEAK_ENCRYPTION) {
      if (pattern.test(text)) {
        return this.makeFinding({
          toolName,
          severity,
          message: t("finding_crypto_weak_encryption", {
            tool: toolName,
            algo: name,
          }),
          evidence: {
            ...
            risk: t("risk_crypto_broken", { algo: name }),
          },
    ...
    for (const { name, pattern } of WEAK_HASHES) {
      if (pattern.test(text)) {
        return this.makeFinding({
          toolName,
          severity: "critical",
          message: t("finding_crypto_weak_hashing", {
            tool: toolName,
            algo: name,
          }),
          evidence: {
            algorithm: name,
            matched_in: "description",
            risk: t("risk_crypto_collision", { algo: name }),
          },
    ...
    for (const { method, pattern } of INSECURE_PRNGS) {
      if (pattern.test(text)) {
        return this.makeFinding({
          toolName,
          severity: "high",
          message: t("finding_crypto_insecure_random", {
            tool: toolName,
            method,
          }),
          evidence: {
            method,
            matched_in: "description",
            risk: t("risk_crypto_predictable"),
          },
    ...
    for (const [paramName, raw] of Object.entries(schema.properties)) {
      ...
      if (weakFound.length > 0) {
        return this.makeFinding({
          ...
          evidence: {
            parameter: paramName,
            weak_options: weakFound,
            risk: t("risk_crypto_weak_selection"),
          },
      ...
      if (minimum !== null && minimum < MIN_KEY_BITS) {
        return this.makeFinding({
          ...
          evidence: {
            ...
            safe_minimum: MIN_KEY_BITS,
            risk: t("risk_crypto_brute_force"),
          },
  ...
  private detectUnspecifiedAlgorithm(
    ...
    return this.makeFinding({
      ...
      evidence: {
        matched_in: "description",
        risk: t("unclear_if_strong_cryptography_is_used"),
      },
