...
export class InsecureSessionManagementRule implements ISecurityRule {
  ...
  private readonly SESSION_INVALIDATION_KEYWORDS = [
    ...
    'invalidate', 'revoke', 'expire', 'terminate'
  ];
  ...
  evaluate(discovery: DiscoveryResult): SecurityFinding[] {
    ...
    const toolsWithWeakSessions = this.findToolsWithWeakSessions(discovery.tools);

    if (toolsWithWeakSessions.length > 0 && !hasSessionInvalidation) {
      findings.push({
        severity: this.severity,
        message: t('sec_043_no_invalidation', { count: toolsWithWeakSessions.length }),
        component: 'server',
        ruleCode: this.code,
  ...
  private findToolsWithWeakSessions(tools: McpTool[]): McpTool[] {
    return tools.filter(tool => {
      ...
      for (const [propName, propSchema] of Object.entries(properties)) {
        if (this.SESSION_PARAM_NAMES.some(p => p.toLowerCase() === propName.toLowerCase())) {
          const schema = propSchema as { pattern?: string; maxLength?: number };
          const hasPattern = schema.pattern !== undefined;
          const hasLength = schema.maxLength !== undefined && schema.maxLength >= 32;
