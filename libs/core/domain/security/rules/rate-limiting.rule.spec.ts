  ...
  describe("should detect vulnerabilities", () => {
    it("should detect unthrottled expensive operations (Medium)", () => {
      ...
      const finding = findings.find((f) =>
        f.message.toLowerCase().includes("rate limiting not implemented"),
      );
    ...
    it("should detect unthrottled authentication (High)", () => {
      ...
      const highFinding = findings.find(
        (f) =>
          f.message.toLowerCase().includes("must implement") &&
          f.message.toLowerCase().includes("rate limiting"),
      );
      expect(highFinding).toBeDefined();
      expect(highFinding!.severity).toBe("high");

      // Might also find the generic "rate limiting not implemented" one
      const genericFinding = findings.find((f) =>
        f.message.toLowerCase().includes("rate limiting not implemented"),
      );
    ...
    it("should detect missing file size limits (Medium)", () => {
      ...
      const findings = rule.evaluate(discovery);
      const sizeFinding = findings.find((f) =>
        f.message.includes("lacks size limit"),
      );
        ...
        tools: [
          {
            name: "safe_api_call",
            description:
              "Fetches external data. Rate limit: 60 requests per minute.",
            inputSchema: {
              type: "object",
              properties: {
                url: { type: "string" },
