...
export class DataLeakageRule implements ISecurityRule {
  ...
  get name() { return t('sec_data_leakage_name'); }
  get description() { return t('sec_data_leakage_desc'); }
  ...
  private readonly SENSITIVE_KEYWORDS = [
    ...
    'private_key', 'auth_code', 'credential', 'ssn', 'credit_card'
  ];

  private readonly RISKY_RESOURCE_EXTENSIONS = [
    '.env', '.key', '.pem', '.p12', '.pfx', '.kdbx', 'id_rsa', 'config.json', 'settings.xml'
  ];
  ...
  private analyzeTool(tool: McpTool): SecurityFinding[] {
    ...
    for (const [paramName, paramConfig] of Object.entries(tool.inputSchema.properties)) {
      ...
      if (this.isSensitive(paramName, typeof config.description === 'string' ? config.description : undefined)) {
        ...
        findings.push({
          severity: 'medium',
          message: t('finding_data_leakage_sensitive', { tool: tool.name, param: paramName }),
  ...
  private analyzeResource(resource: McpResource): SecurityFinding[] {
    ...
    const isRiskyFile = this.RISKY_RESOURCE_EXTENSIONS.some(ext =>
      uri.endsWith(ext) || name.includes(ext)
    );

    if (isRiskyFile) {
      findings.push({
        severity: 'critical',
        message: t('finding_data_leakage_resource', { resource: resource.name || t('unknown') }),
        component: `resource:${resource.name || t('unknown')}`,
        ruleCode: this.code,
        location: { type: 'resource', uri: resource.uri },
        evidence: { uri: resource.uri, match: t('risk_data_leakage_risky_file') },
