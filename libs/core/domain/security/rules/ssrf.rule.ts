...
export class SSRFDetectionRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_ssrf_name");
  }
  get description() {
    return t("sec_ssrf_desc");
  }
  readonly helpUri =
        ...
        if (weakness.isWeak) {
          findings.push({
            ...
            evidence: {
              ...
              issue: weakness.reason ? t(weakness.reason) : null,
            },
  ...
  private isUrlParameter(
    ...
    return this.URL_INDICATORS.some(
      (i) => nameLower.includes(i) || descLower.includes(i),
    );
  }

  private isWeakUrlPattern(pattern: string): {
    isWeak: boolean;
