...
export class SensitiveDataExposureRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_sensitive_exposure_name");
  }
  get description() {
    return t("sec_sensitive_exposure_desc");
  }
  readonly helpUri =
    ...
    for (const [paramName, paramConfig] of Object.entries(
      ...
      if (config.type === "string") {
        ...
        if (!config.format) {
          findings.push({
            severity,
            message: t("finding_sensitive_no_format", {
              param: paramName,
        ...
        if (!config.pattern && !config.format) {
          findings.push({
            severity,
            message: t("finding_sensitive_no_pattern", {
              param: paramName,
              category: sensitiveCategory,
            }),
            component: `tool:${tool.name}`,
      ...
      if (
        ...
        findings.push({
          severity: "high",
          message: t("finding_sensitive_no_protection", {
            tool: tool.name,
            category: sensitiveCategory,
          }),
          component: `tool:${tool.name}`,
  ...
  private getPatternRecommendation(category: string): string {
    ...
    return (
      ...
      t("implement_strict_validation_pattern_for_this_sensi")
    );
  }
    ...
    if (outputSchema && outputSchema.properties) {
      for (const [propName, propConfig] of Object.entries(
        ...
        if (sensitiveCategory) {
          findings.push({
            severity: "high",
            message: t("finding_sensitive_response", {
              tool: tool.name,
              category: sensitiveCategory,
              prop: propName,
            }),
