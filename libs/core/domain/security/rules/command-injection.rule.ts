...
export class CommandInjectionRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_command_injection_name");
  }
  get description() {
    return t("sec_command_injection_desc");
  }
  readonly helpUri =
    ...
    for (const [paramName, paramConfig] of Object.entries(
      ...
      if ((isExecutionTool || isArgumentParam) && config.type === "string") {
        if (!config.pattern) {
          findings.push({
            severity: "critical",
            message: t("finding_cmd_injection_no_validation", {
              param: paramName,
              tool: tool.name,
            }),
            component: `tool:${tool.name}`,
          ...
          if (weakResult.isWeak) {
            findings.push({
              severity: "critical",
              message: t("finding_cmd_injection_weak_validation", {
                param: paramName,
              }),
              component: `tool:${tool.name}`,
              ruleCode: this.code,
