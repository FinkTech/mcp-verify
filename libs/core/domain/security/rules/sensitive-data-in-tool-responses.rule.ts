...
export class SensitiveDataInToolResponsesRule implements ISecurityRule {
  ...
  private readonly PII_KEYWORDS = [
    ...
    'address', 'birthday', 'birth_date', 'dob', 'gender',
    'race', 'ethnicity', 'religion', 'medical', 'health'
  ];

  private readonly REDACTION_KEYWORDS = [
    'redacted', 'filtered', 'sanitized', 'masked', 'anonymized',
    'encrypted', 'hashed', 'obfuscated', 'protected', 'secure'
  ];

  private readonly SENSITIVE_TOOL_PATTERNS = [
    /get.*user/i, /fetch.*profile/i, /retrieve.*account/i,
    /list.*credentials?/i, /show.*secrets?/i, /get.*password/i,
    /fetch.*token/i, /retrieve.*keys?/i, /list.*api.*key/i
  ];
  ...
  private hasOutputConstraints(tool: McpTool): boolean {
    ...
    if (tool.description) {
      const descLower = tool.description.toLowerCase();
      return descLower.includes('output') && this.REDACTION_KEYWORDS.some(kw =>
