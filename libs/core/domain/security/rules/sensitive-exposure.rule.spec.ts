...
describe('SensitiveDataExposureRule', () => {
    ...
    describe('should detect vulnerabilities', () => {
        it('should detect sensitive parameters without format specification (Critical)', () => {
            ...
            const finding = findings.find(f => f.message.includes('lacks format specification'));
        ...
        it('should warn about potential logging of sensitive data (Medium)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const logFinding = findings.find(f => f.message.toLowerCase().includes('might be logged'));
        ...
        it('should detect missing data protection measures (High)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const protectionFinding = findings.find(f => f.message.toLowerCase().includes('without protection'));
        ...
        it('should detect sensitive data in output schema (High)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const outputFindings = findings.filter(f => f.message.toLowerCase().includes('returns') && f.message.toLowerCase().includes('data'));
        ...
        it('should pass if proper format and encryption are used', () => {
            const discovery: DiscoveryResult = {
                tools: [
                    {
                        name: 'safe_login',
                        description: 'Login using secure encryption (TLS) and hashed storage.',
                        inputSchema: {
                            type: 'object',
                            properties: {
                                password: {
