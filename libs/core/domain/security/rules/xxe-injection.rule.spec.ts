...
describe("XXEInjectionRule", () => {
  ...
  describe("should detect vulnerabilities", () => {
    it("should detect dangerous DTD configuration (Critical)", () => {
      ...
      expect(
        (findings[0].evidence as any).configuration.toLowerCase(),
      ).toContain("dtd processing enabled");
    });
    ...
    it("should detect XML tools without safe configuration (High)", () => {
      ...
      const paramFinding = findings.find(
        (f) => f.location?.parameter === "content",
      );
      ...
      expect(toolFinding!.message).toContain(
        "does not explicitly disable external entities",
      );
    });
    ...
    it("should detect file uploads allowing XML (Critical)", () => {
      ...
      const findings = rule.evaluate(discovery);
      const fileFinding = findings.find((f) =>
        f.message.includes("accepts XML file uploads"),
      );
        ...
        tools: [
          {
            name: "safe_parser",
            description:
              "Parses XML. Note: disable external entities is set to true.",
            inputSchema: {
              type: "object",
              properties: {
                xml_data: {
