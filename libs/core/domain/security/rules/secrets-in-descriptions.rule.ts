...
export class SecretsInDescriptionsRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_secrets_desc_name");
  }
  get description() {
    return t("sec_secrets_desc_desc");
  }
  readonly helpUri =
  ...
  private readonly SENSITIVE_PATTERNS = {
    ...
    internalPath: {
      pattern:
        /(?:\/home\/|\/var\/|\/etc\/|C:\\Users\\|C:\\Program Files\\)[^\s'"]{10,}/gi,
      severity: "medium" as const,
  ...
  private scanText(
    ...
    for (const [patternName, { pattern, severity, type }] of Object.entries(
      ...
      if (matches && matches.length > 0) {
        ...
        const redactedMatches = matches.map((match) =>
          this.redactSensitiveData(match, type),
        );
