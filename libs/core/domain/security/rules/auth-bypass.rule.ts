...
export class AuthenticationBypassRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_auth_bypass_name");
  }
  get description() {
    return t("sec_auth_bypass_desc");
  }
  readonly helpUri =
   ...
   */
  private readonly STRONG_HASHING = ["bcrypt", "argon2", "scrypt", "pbkdf2"];
  ...
  private isPasswordParameter(
    ...
    return (
      passwordNames.some(
        (p) => nameLower.includes(p) || descLower.includes("password"),
      ) || config.format === "password"
    );
  }

  private isUsernameParameter(
    name: string,
    config: Record<string, JsonValue>,
  ): boolean {
    ...
    return (
      usernameNames.some((u) => nameLower.includes(u)) ||
      config.format === "email"
    );
  }

  private isCredentialParameter(
    name: string,
    config: Record<string, JsonValue>,
  ): boolean {
