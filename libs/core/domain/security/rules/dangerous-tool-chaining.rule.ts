...
export class DangerousToolChainingRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_tool_chaining_name");
  }
  get description() {
    return t("sec_tool_chaining_desc");
  }
  readonly helpUri =
  ...
  private isCodeGenerationTool(tool: McpTool): boolean {
    ...
    if (
      this.CODE_GEN_TOOL_NAMES.some((pattern) =>
  ...
  private analyzeCodeGenTool(tool: McpTool): SecurityFinding[] {
    ...
    const hasSafetyKeyword = this.SAFETY_KEYWORDS.some((kw) =>
      text.includes(kw),
    );
