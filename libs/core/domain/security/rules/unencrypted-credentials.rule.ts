...
export class UnencryptedCredentialsRule implements ISecurityRule {
  ...
  get name() { return t('sec_unencrypted_creds_name'); }
  get description() { return t('sec_unencrypted_creds_desc'); }
  ...
  private readonly SECURITY_KEYWORDS = [
    ...
    'hash', 'hashed', 'hashing', 'bcrypt', 'scrypt', 'argon2',
    'secure', 'secured', 'securely',
    'keychain', 'vault', 'keystore',
    'aes', 'rsa', 'cipher',
    'protected', 'safeguard'
  ];
  ...
  private readonly INSECURE_STORAGE_METHODS = [
    'plaintext', 'plain text', 'cleartext', 'clear text',
    'unencrypted', 'raw', 'base64' // base64 is encoding, not encryption
  ];
  ...
  private analyzeTool(tool: McpTool): SecurityFinding[] {
    ...
    const hasStorageKeyword = this.STORAGE_KEYWORDS.some(kw => text.includes(kw));
    ...
    const credentialKeywords = this.CREDENTIAL_KEYWORDS.filter(kw => text.includes(kw));
    ...
    const hasSecurityKeyword = this.SECURITY_KEYWORDS.some(kw => text.includes(kw));

    // Check for explicit insecure storage mentions
    const insecureStorageMethods = this.INSECURE_STORAGE_METHODS.filter(kw => text.includes(kw));
    ...
    if (tool.inputSchema?.properties) {
      for (const [paramName, paramConfig] of Object.entries(tool.inputSchema.properties)) {
        ...
        const isCredentialParam = this.CREDENTIAL_KEYWORDS.some(kw => paramLower.includes(kw));
        const isStorageParam = this.STORAGE_KEYWORDS.some(kw => paramLower.includes(kw));
        ...
        if (isCredentialParam && isStorageParam) {
          ...
          const hasParamSecurity = this.SECURITY_KEYWORDS.some(kw => paramDesc.includes(kw));
