...
export class RateLimitingRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_rate_limiting_name");
  }
  get description() {
    return t("sec_rate_limiting_desc");
  }
  ...
  private analyzeTool(tool: McpTool): SecurityFinding[] {
    ...
    const expensiveCategories = this.getExpensiveCategories(
      tool.name,
      tool.description,
    );
    ...
    if (!hasRateLimiting) {
      ...
      findings.push({
        severity,
        message: t("finding_rate_limit_missing", {
          tool: tool.name,
          ops: expensiveCategories.join(", "),
        }),
  ...
  private hasRateLimiting(tool: McpTool): boolean {
    ...
    if (tool.description) {
      ...
      if (
        this.RATE_LIMIT_INDICATORS.some((indicator) =>
  ...
  private getFileParameters(
    properties: Record<string, JsonValue>,
  ): Array<{ name: string; config: Record<string, JsonValue> }> {
