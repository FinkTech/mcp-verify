...
const DANGEROUS_PATTERNS = [
  ...
  {
    pattern: /curl\s+[^\s]+\s*\|\s*(bash|sh|zsh|fish)/i,
    description: "curl piped to shell",
  },
  {
    pattern: /wget\s+[^\s]+\s*\|\s*(bash|sh|zsh|fish)/i,
    description: "wget piped to shell",
  },
  {
    pattern: /fetch\s+[^\s]+\s*\|\s*(bash|sh|zsh|fish)/i,
    description: "fetch piped to shell",
  },
  ...
  {
    pattern: /\/bin\/(ba)?sh\s+-i\s*>/i,
    description: "interactive shell redirection",
  },
  { pattern: /mkfifo.*nc.*sh/i, description: "named pipe reverse shell" },

  // Remote payload execution
  {
    pattern: /curl.*https?:\/\/.*\.(sh|bash|py|pl|rb)/i,
    description: "remote script download",
  },
  {
    pattern: /wget.*https?:\/\/.*\.(sh|bash|py|pl|rb)/i,
    description: "remote script download",
  },

  // Command chaining (suspicious in MCP context)
  {
    pattern: /;\s*(curl|wget|nc|bash|sh|rm|dd)/i,
    description: "command chaining with dangerous command",
  },
  {
    pattern: /&&\s*(curl|wget|nc|bash|sh|rm|dd)/i,
    description: "AND chaining with dangerous command",
  },
  {
    pattern: /\|\|\s*(curl|wget|nc|bash|sh|rm|dd)/i,
    description: "OR chaining with dangerous command",
  },

  // Command substitution
  {
    pattern: /\$\(.*(?:curl|wget|nc|bash|sh|eval)\)/i,
  ...
  // Obfuscation
  {
    pattern: /base64\s+-d.*\|\s*(bash|sh|eval)/i,
    description: "base64 decode piped to execution",
  },
  {
    pattern: /echo.*\|\s*base64\s+-d.*\|\s*(bash|sh)/i,
    description: "base64 encoded command execution",
  },
  {
    pattern: /xxd.*\|\s*(bash|sh|eval)/i,
    description: "hex decode to execution",
  },
  ...
  {
    pattern: /rm\s+-rf\s+\//,
    description: "recursive forced delete from root",
  },
  ...
  {
    pattern: /Invoke-WebRequest.*\|\s*Invoke-Expression/i,
    description: "PowerShell IWR piped to IEX",
  },
  {
    pattern: /iwr\s+.*\|\s*iex/i,
    description: "PowerShell iwr piped to iex (alias)",
  },
  {
    pattern: /Invoke-Expression.*\(.*Invoke-WebRequest/i,
    description: "PowerShell nested IEX with IWR",
  },
  {
    pattern: /wget.*\|\s*iex/i,
    description: "PowerShell wget alias piped to iex",
  },
  {
    pattern: /curl.*\|\s*iex/i,
    description: "PowerShell curl alias piped to iex",
  },

  // Certutil abuse (binary download/decode)
  {
    pattern: /certutil.*-urlcache.*http/i,
    description: "certutil binary download abuse",
  },
  { pattern: /certutil.*-decode/i, description: "certutil decode abuse" },

  // Bitsadmin abuse (binary download)
  {
    pattern: /bitsadmin.*\/transfer/i,
    description: "bitsadmin binary download",
  },
  { pattern: /bitsadmin.*\/download/i, description: "bitsadmin file download" },

  // PowerShell reverse shells
  {
    pattern: /\$client\s*=\s*New-Object.*Net\.Sockets\.TcpClient/i,
    description: "PowerShell TCP reverse shell",
  },
  {
    pattern: /New-Object.*System\.Net\.Sockets\.TcpClient/i,
    description: "PowerShell socket-based reverse shell",
  },
  {
    pattern: /\$stream\s*=\s*\$client\.GetStream/i,
    description: "PowerShell stream-based reverse shell",
  },

  // PowerShell obfuscation
  { pattern: /FromBase64String/i, description: "PowerShell base64 decode" },
  {
    pattern: /-enc(odedcommand)?\s+[A-Za-z0-9+/=]{20,}/i,
    description: "PowerShell encoded command execution",
  },
  {
    pattern: /\[System\.Text\.Encoding\]::UTF8\.GetString/i,
    description: "PowerShell string decoding",
  },
  ...
  {
    pattern: /rd\s+\/s\s+\/q\s+C:\\/i,
    description: "CMD recursive directory delete",
  },
  {
    pattern: /del\s+\/f\s+\/s\s+\/q/i,
    description: "CMD forced file deletion",
  },
  ...
  {
    pattern: /Set-ItemProperty.*HKLM/i,
    description: "PowerShell HKLM registry write",
  },

  // Windows scheduled tasks (persistence)
  {
    pattern: /schtasks.*\/create/i,
    description: "Windows scheduled task creation",
  },
  {
    pattern: /Register-ScheduledTask/i,
    description: "PowerShell scheduled task registration",
  },
];
...
export class MaliciousConfigFileRule implements ISecurityRule {
  ...
  evaluateConfigFile(configPath: string): SecurityFinding[] {
    ...
    if (config.mcpServers) {
      for (const [serverName, serverConfig] of Object.entries(
        ...
        const serverFindings = this.analyzeServerConfig(
          serverName,
          serverConfig,
          configPath,
        );
   ...
   */
  private detectSuspiciousEnvVars(
    env: Record<string, string>,
    ...
    for (const [key, value] of Object.entries(env)) {
      if (
        API_ENDPOINT_VARS.some((v) =>
          key.toUpperCase().includes(v.toUpperCase()),
        )
      ) {
