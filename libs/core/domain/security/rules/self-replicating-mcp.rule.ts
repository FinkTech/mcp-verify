...
export class SelfReplicatingMcpRule implements ISecurityRule {
  ...
  private readonly REPLICATION_KEYWORDS = [
    'replicate', 'spread', 'propagate', 'clone', 'duplicate',
    'copy.*self', 'install.*self', 'deploy.*self',
    'worm', 'virus', 'infect', 'contaminate'
  ];

  private readonly DEPLOYMENT_PATTERNS = [
    /install.*mcp.*server/i, /deploy.*mcp.*server/i,
    /setup.*mcp.*server/i, /create.*mcp.*server/i,
    /spawn.*server/i, /launch.*remote.*server/i,
    /distribute.*server/i
  ];

  private readonly SCANNING_KEYWORDS = [
    'scan.*network', 'scan.*hosts', 'find.*vulnerable',
    'discover.*targets', 'enumerate.*systems', 'probe.*network',
    'port.*scan', 'host.*discovery', 'network.*recon'
  ];
  ...
  private hasRemoteDeployment(tool: McpTool): boolean {
    ...
    if (tool.inputSchema?.properties) {
      for (const propName of Object.keys(tool.inputSchema.properties)) {
        ...
        if (propLower.includes('remote') || propLower.includes('target') || propLower.includes('host')) {
          // If tool also mentions deployment/installation
          const nameLower = tool.name.toLowerCase();
          const descLower = tool.description?.toLowerCase() || '';

          const mentionsDeployment = nameLower.includes('install') || nameLower.includes('deploy') ||
            descLower.includes('install') || descLower.includes('deploy');
