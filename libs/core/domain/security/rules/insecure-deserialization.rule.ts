...
export class InsecureDeserializationRule implements ISecurityRule {
  ...
  get name() { return t('sec_insecure_deserialization_name'); }
  get description() { return t('sec_insecure_deserialization_desc'); }
  readonly helpUri = 'https://owasp.org/www-project-top-ten/2017/A8_2017-Insecure_Deserialization';
  ...
  private readonly DESERIALIZATION_KEYWORDS = [
    'deserialize', 'unserialize', 'unpickle', 'unmarshal',
    'decode', 'parse', 'load', 'restore', 'reconstruct'
  ];
  ...
  private readonly DANGEROUS_FORMATS = [
    'pickle', 'cpickle', 'yaml', 'java', 'php', 'ruby marshal',
    'native', 'binary', '.net', 'objectinputstream'
  ];
  ...
  private readonly SAFE_FORMATS = [
    'json', 'xml', 'protobuf', 'msgpack', 'avro', 'thrift'
  ];
  ...
  private readonly SAFE_INDICATORS = [
    'schema validation', 'type checking', 'whitelist', 'signature',
    'hmac', 'signed', 'verified', 'safe_load', 'limited deserialization'
  ];

  /**
   * Dangerous YAML methods.
   */
  private readonly UNSAFE_YAML = ['yaml.load', 'yaml.unsafe_load', 'yaml.fullload'];
  ...
  private analyzeTool(tool: McpTool): SecurityFinding[] {
    const findings: SecurityFinding[] = [];
    const isDeserializationTool = this.isDeserializationTool(tool.name, tool.description);
    ...
    const dangerousFormat = this.detectDangerousFormat(tool.name, tool.description);

    // CRITICAL: Tool uses dangerous serialization format
    if (dangerousFormat) {
      findings.push({
        severity: 'critical',
        message: t('finding_deserialization_dangerous', { tool: tool.name, format: dangerousFormat }),
        ...
        remediation: t('remediation_deserialization_safe', { format: dangerousFormat })
      });
    }
    ...
    for (const [paramName, paramConfig] of Object.entries(tool.inputSchema.properties)) {
      ...
      if (this.isSerializedDataParam(paramName, config)) {
        ...
        if (config.type === 'object' && !config.properties && !config.additionalProperties) {
          findings.push({
            severity: 'high',
            message: t('finding_deserialization_arbitrary', { param: paramName }),
            component: `tool:${tool.name}`,
            ruleCode: this.code,
            location: { type: 'tool', name: tool.name, parameter: paramName },
            evidence: {
    ...
    if (!hasSafeIndicators && findings.some(f => f.severity === 'critical' || f.severity === 'high')) {
      findings.push({
        severity: 'high',
  ...
  private detectDangerousFormat(name: string, description?: string): string | null {
    const text = `${name} ${description || ''}`.toLowerCase();
  ...
  private detectUnsafeYAML(description?: string): boolean {
    ...
    return this.UNSAFE_YAML.some(method => text.includes(method.toLowerCase()));
  }

  private isSerializedDataParam(name: string, config: Record<string, JsonValue>): boolean {
    const serializedParamNames = [
      'data', 'object', 'payload', 'serialized', 'encoded',
      'pickled', 'marshaled', 'blob', 'binary'
    ];

    const nameLower = name.toLowerCase();
    const descLower = (typeof config.description === 'string' ? config.description : '').toLowerCase();

    return serializedParamNames.some(sp =>
      nameLower.includes(sp) || descLower.includes(sp)
    ) || this.DESERIALIZATION_KEYWORDS.some(kw =>
