...
export class AutonomousMcpBackdoorRule implements ISecurityRule {
  ...
  private readonly SELF_MODIFICATION_PATTERNS = [
    /update.*self/i, /upgrade.*self/i, /patch.*self/i,
    /modify.*code/i, /rewrite.*self/i, /install.*dependency/i,
    /exec.*code/i, /eval.*code/i, /compile.*self/i
  ];

  private readonly STEALTH_KEYWORDS = [
    'stealth', 'hidden', 'covert', 'secret', 'invisible',
    'background', 'silent', 'unattended', 'autonomous',
    'scheduled', 'cron', 'periodic', 'interval'
  ];

  private readonly BACKDOOR_PATTERNS = [
    /reverse.*shell/i, /command.*control/i, /c2/i, /c&c/i,
    /beacon/i, /phone.*home/i, /exfiltrate/i, /tunnel/i,
    /proxy.*traffic/i, /relay/i
  ];
  ...
  private hasStealthBehavior(tool: McpTool): boolean {
    ...
    return this.STEALTH_KEYWORDS.some(keyword =>
