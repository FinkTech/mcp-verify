...
describe('SSRFDetectionRule', () => {
  ...
  describe('should detect vulnerabilities', () => {
    it('should detect URL parameters without validation pattern (High)', () => {
      ...
      expect(findings[0].message).toContain('appears to be a URL input without validation');
    });
    ...
    it('should detect unanchored patterns (Medium)', () => {
      ...
      expect(((findings[0].evidence as any).issue as string).toLowerCase()).toContain('not anchored');
    });
    ...
    it('should detect patterns allowing insecure HTTP (Medium)', () => {
      ...
      expect(findings.length).toBeGreaterThan(0);
      expect(findings[0].severity).toBe('medium');
      expect(((findings[0].evidence as any).issue as string).toLowerCase()).toContain('allows insecure http');
    });
    ...
    it('should detect permissive wildcard patterns (Medium)', () => {
      ...
      expect(findings.length).toBeGreaterThan(0);
      expect(findings[0].severity).toBe('medium');
      expect(((findings[0].evidence as any).issue as string).toLowerCase()).toContain('starts with wildcard');
    });
  });
