...
export class WeaponizedMcpFuzzerRule implements ISecurityRule {
  ...
  private readonly OFFENSIVE_TOOL_PATTERNS = [
    /fuzz/i, /exploit/i, /attack/i, /penetrate/i, /pentest/i,
    /hack/i, /crack/i, /break/i, /breach/i, /pwn/i,
    /payload/i, /shellcode/i, /backdoor/i, /rootkit/i
  ];

  private readonly PAYLOAD_KEYWORDS = [
    'payload', 'injection', 'xss', 'sqli', 'rce',
    'command_injection', 'path_traversal', 'xxe',
    'ssrf', 'deserialization', 'idor'
  ];

  private readonly AUTHORIZATION_KEYWORDS = [
    'authorized', 'authorization', 'consent', 'permission',
    'approved', 'whitelisted', 'allowed', 'verified'
  ];
  ...
  private isOffensiveSecurityTool(tool: McpTool): boolean {
    ...
    if (tool.inputSchema?.properties) {
      for (const propName of Object.keys(tool.inputSchema.properties)) {
        ...
        if (propLower.includes('target') || propLower.includes('endpoint') || propLower.includes('victim')) {
          return true;
        }
      }
    }
  ...
  private hasAuthorizationCheck(tool: McpTool): boolean {
    ...
    if (tool.inputSchema?.properties) {
      for (const propName of Object.keys(tool.inputSchema.properties)) {
        const propLower = propName.toLowerCase();
        if (propLower.includes('authorization') || propLower.includes('consent') || propLower.includes('approved')) {
          return true;
        }
      }
    }
