...
describe('AuthenticationBypassRule', () => {
    ...
    describe('should detect vulnerabilities', () => {
        ...
        it('should detect tools using weak hashing algorithms', () => {
            ...
            expect(findings[0].message.toLowerCase()).toContain('uses weak hashing algorithms');
        });
        ...
        it('should detect password parameters without minimum length', () => {
            ...
            const findings = rule.evaluate(discovery);
            expect(findings.length).toBeGreaterThan(0);
            const lengthFinding = findings.find(f => f.message.includes('minimum length'));
        ...
        it('should detect authentication tools that do not mention hashing', () => {
            ...
            const findings = rule.evaluate(discovery);
            const hashFinding = findings.find(f => f.message.toLowerCase().includes('does not') && f.message.toLowerCase().includes('hashing'));
        ...
        it('should detect potential username enumeration (Medium)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const enumFinding = findings.find(f => f.message.includes('user enumeration'));
        ...
        it('should detect credential parameters transmitted as plain strings (Medium)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const plainFinding = findings.find(f => f.message.toLowerCase().includes('plain text credentials'));
        ...
        it('should detect missing brute-force protection (Medium)', () => {
            ...
            const findings = rule.evaluate(discovery);
            const bruteForceFinding = findings.find(f => f.message.toLowerCase().includes('lacks brute force protection'));
        ...
        it('should pass for tools using strong hashing and strong password policies', () => {
            const discovery: DiscoveryResult = {
                tools: [
                    {
                        name: 'create_admin',
                        description: 'Creates admin. Passwords hashed with Argon2id. Implements rate limiting and brute force protection.',
                        inputSchema: {
                            type: 'object',
                            properties: {
                                username: { type: 'string' },
