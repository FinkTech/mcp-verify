...
export class XXEInjectionRule implements ISecurityRule {
  ...
  get name() {
    return t("sec_xxe_name");
  }
  get description() {
    return t("sec_xxe_desc");
  }
  readonly helpUri =
  ...
  private analyzeTool(tool: McpTool): SecurityFinding[] {
    ...
    for (const [paramName, paramConfig] of Object.entries(
      ...
      if (this.isXMLParameter(paramName, config)) {
        ...
        if (
          paramName.toLowerCase().includes("svg") ||
          paramName.toLowerCase().includes("image")
        ) {
          if (!hasSafeConfig) {
            findings.push({
              severity: "high",
  ...
  private isXMLParameter(
    ...
    return (
      xmlParamNames.some(
        (xp) => nameLower.includes(xp) || descLower.includes("xml"),
      ) || config.format === "xml"
    );
  }

  private isFileUploadParam(
    name: string,
    config: Record<string, JsonValue>,
  ): boolean {
    ...
    return (
      uploadNames.some((un) => nameLower.includes(un)) ||
  ...
  private getAcceptedExtensions(config: Record<string, JsonValue>): string[] {
    ...
    if (typeof config.description === "string") {
      const descMatch = config.description.match(
        /\.(xml|svg|rss|atom|xhtml)/gi,
      );
      if (descMatch) {
