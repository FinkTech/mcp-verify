...
import type { DiscoveryResult, SecurityFinding, SecurityReport } from '../mcp-server/entities/validation.types';
...
import type { McpVerifyConfig, SecurityRuleBlock } from '../config/config.types';
...
export class SecurityScanner {
   ...
   */
  private filterIgnoredFindings(findings: SecurityFinding[]): SecurityFinding[] {
    ...
    return findings.filter(finding => {
      ...
      const shouldIgnore = McpIgnoreParser.shouldIgnore(ruleCode, toolName, this.ignoreRules);
  ...
  scan(discovery: DiscoveryResult): SecurityReport {
    ...
    this.runLegacyHeuristics(discovery, findings, (penalty) => score -= penalty);
  ...
  private runLegacyHeuristics(discovery: DiscoveryResult, findings: SecurityFinding[], deductScore: (n: number) => void) {
    const riskPatterns = [
      { regex: /exec|shell|bash|cmd|system|run|eval/i, penalty: 30, severity: 'critical', key: 'sec_heuristic_rce' as const },
      { regex: /auth|login|password|key|token|secret/i, penalty: 20, severity: 'high', key: 'sec_heuristic_auth' as const },
      { regex: /write|delete|remove|fs|file|upload/i, penalty: 15, severity: 'medium', key: 'sec_heuristic_fs' as const },
      { regex: /sql|query|db|database|drop/i, penalty: 15, severity: 'medium', key: 'sec_heuristic_db' as const },
      { regex: /curl|fetch|request|http|proxy/i, penalty: 10, severity: 'low', key: 'sec_heuristic_net' as const },
    ] as const;

    if (discovery.tools) {
      discovery.tools.forEach(tool => {
        riskPatterns.forEach(pattern => {
          if (pattern.regex.test(tool.name) || (tool.description && pattern.regex.test(tool.description))) {
  ...
  scanUnified(
    ...
    if (fuzzerFindings && fuzzerFindings.length > 0) {
      for (const fuzzerFinding of fuzzerFindings) {
        ...
        const securityFinding: SecurityFinding = {
          ...
          remediation: fuzzerFinding.remediation || t('remediation_fuzzer_confirmed')
        };

        // Mark as fuzzer finding for reporting
        (securityFinding as SecurityFinding & { source?: string }).source = 'fuzzer';

        findings.push(securityFinding);

        // Calculate weighted fuzzer penalty
        const basePenalty = STATIC_SEVERITY_PENALTIES[fuzzerFinding.severity] || 0;
        const confidenceWeight = FUZZER_CONFIDENCE_WEIGHTS[fuzzerFinding.confidence] || 0.5;
        const weightedPenalty = basePenalty * FUZZER_WEIGHT_MULTIPLIER * confidenceWeight;
    ...
    const staticFindingsCount = filteredFindings.filter(
      f => (f as SecurityFinding & { source?: string }).source === 'static' ||
  ...
  static convertFuzzerVulnerabilities(
    ...
    return vulnerabilities.map(v => ({
      ...
      severity: (v.severity as 'critical' | 'high' | 'medium' | 'low' | 'info') || 'medium',
