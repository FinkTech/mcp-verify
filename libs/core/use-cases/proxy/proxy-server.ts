...
import type { JsonValue, JsonRpcMessage, JsonObject, ToolCallParams } from '../../domain/shared/common.types';
...
import type { SecurityFinding, DiscoveryResult } from '../../domain/mcp-server/entities/validation.types';
...
export type AuditEventType = 'start' | 'request' | 'block' | 'response' | 'error' | 'security-analysis' | 'rate-limit-backoff' | 'panic-mode-activated';
...
export class McpProxyServer extends EventEmitter {
  ...
  private ruleCache = new Map<string, { findings: SecurityFinding[]; timestamp: number }>();
  ...
  async stop(): Promise<void> {
    ...
    for (const guardrail of this.guardrails) {
      ...
      if ('destroy' in guardrail && typeof (guardrail as any).destroy === 'function') {
              ...
              JSON.stringify({
                ...
                error: {
                  ...
                  data: {
                    ...
                    clientId,
                    blockedUntil: clientState ? new Date(clientState.blockedUntil).toISOString() : null,
                    remainingSeconds: clientState ? Math.ceil((clientState.blockedUntil - Date.now()) / 1000) : 0,
              ...
              JSON.stringify({
                ...
                error: {
                  ...
                  data: finding ? {
                    ...
                    remediation: finding.remediation ?? 'Consult security documentation for remediation guidance',
   ...
   */
  private async runSecurityAnalysis(message: JsonRpcMessage): Promise<InterceptResult> {
    const startTime = Date.now();

    // ── LAYER 1: Fast Rules (<10ms) - Pattern Matching ──────────────────────
    const layer1Findings = await this.runFastRulesWithCache(message);
    const criticalL1 = layer1Findings.filter(f => f.severity === 'critical' || f.severity === 'high');
    ...
    if (this.isSuspiciousTool(message)) {
      ...
      const criticalL2 = layer2Findings.filter(f => f.severity === 'critical' || f.severity === 'high');
    ...
    if (this.enableDeepAnalysis && this.requiresDeepAnalysis(message)) {
      ...
      const criticalL3 = layer3Findings.filter(f => f.severity === 'critical' || f.severity === 'high');
   ...
   */
  private async runFastRules(message: JsonRpcMessage): Promise<SecurityFinding[]> {
   ...
   */
  private detectSQLInjection(toolName: string, args: JsonObject): SecurityFinding[] {
    const findings: SecurityFinding[] = [];

    // SQL keywords in tool name
    const sqlKeywords = ['sql', 'query', 'database', 'db', 'mysql', 'postgres', 'select'];
    const isSQLTool = sqlKeywords.some(kw => toolName.toLowerCase().includes(kw));
        ...
        for (const pattern of sqlPatterns) {
          if (pattern.test(value)) {
            findings.push({
              ...
              remediation: 'Use parameterized queries with placeholders. Never concatenate user input into SQL strings.',
            });
   ...
   */
  private detectCommandInjection(toolName: string, args: JsonObject): SecurityFinding[] {
    const findings: SecurityFinding[] = [];
      ...
      if (typeof value === 'string') {
        ...
        for (const pattern of cmdPatterns) {
          if (pattern.test(value)) {
            findings.push({
              ...
              component: `tool:${toolName}`,
              location: { type: 'tool', name: toolName, parameter: key },
              evidence: { parameter: key, value: value.substring(0, 100), pattern: pattern.source },
              remediation: 'Use safe command execution APIs with argument arrays. Avoid shell=true.',
            });
            break;
          }
        }
        ...
        if (dangerousCommands.test(value)) {
          findings.push({
            ...
            remediation: 'Remove dangerous file system commands. Use safe, scoped operations.',
          });
        }
      }
    }
  ...
  private detectSSRF(toolName: string, args: JsonObject): SecurityFinding[] {
    ...
    const isURLTool = urlKeywords.some(kw => toolName.toLowerCase().includes(kw));
        ...
        for (const pattern of ssrfPatterns) {
          if (pattern.test(value)) {
            findings.push({
              ...
              location: { type: 'tool', name: toolName, parameter: key },
              evidence: { parameter: key, value: value.substring(0, 100) },
              remediation: 'Validate and whitelist allowed external URLs. Block internal IPs.',
            });
            break;
          }
        }
   ...
   */
  private detectPromptInjection(toolName: string, args: JsonObject): SecurityFinding[] {
    const findings: SecurityFinding[] = [];

    const promptKeywords = ['prompt', 'generate', 'llm', 'ai', 'chat', 'completion'];
    const isPromptTool = promptKeywords.some(kw => toolName.toLowerCase().includes(kw));
        ...
        for (const pattern of injectionPatterns) {
          if (pattern.test(value)) {
            findings.push({
              ...
              remediation: 'Sanitize user input before inserting into prompts. Use delimiters and content filtering.',
            });
            break;
          }
        }
   ...
   */
  private detectPathTraversal(toolName: string, args: JsonObject): SecurityFinding[] {
    const findings: SecurityFinding[] = [];

    const fileKeywords = ['file', 'path', 'read', 'write', 'open', 'load'];
    const isFileTool = fileKeywords.some(kw => toolName.toLowerCase().includes(kw));
        ...
        for (const pattern of traversalPatterns) {
          if (pattern.test(value)) {
            findings.push({
              ...
              location: { type: 'tool', name: toolName, parameter: key },
              evidence: { parameter: key, value: value.substring(0, 100) },
              remediation: 'Validate file paths against whitelist. Use path.resolve() and reject paths outside allowed directories.',
            });
            break;
          }
        }
   ...
   */
  private async runFastRulesWithCache(message: JsonRpcMessage): Promise<SecurityFinding[]> {
   ...
   */
  private async runSuspiciousRules(message: JsonRpcMessage): Promise<SecurityFinding[]> {
    const findings: SecurityFinding[] = [];
   ...
   */
  private detectExcessiveAgency(toolName: string, args: JsonObject): SecurityFinding[] {
    const findings: SecurityFinding[] = [];

    const destructiveKeywords = ['delete', 'destroy', 'drop', 'remove', 'erase', 'wipe', 'purge'];
    const isDestructiveTool = destructiveKeywords.some(kw => toolName.toLowerCase().includes(kw));

    if (!isDestructiveTool) return findings;

    // Check for NEGATIVE confirmation patterns (skip, bypass, force)
    const hasNegativeConfirmation = Object.entries(args).some(([key, value]) => {
      const keyLower = key.toLowerCase();
      return (keyLower.includes('skip') || keyLower.includes('bypass') || keyLower.includes('force')) && value === true;
    });

    // Check for POSITIVE confirmation (confirm, verified, acknowledged)
    const hasPositiveConfirmation = Object.entries(args).some(([key, value]) => {
      const keyLower = key.toLowerCase();
      return (keyLower === 'confirm' || keyLower === 'confirmation' || keyLower === 'verified' || keyLower === 'acknowledged') && value === true;
    });
    ...
    if (shouldBlock) {
      findings.push({
        ...
        remediation: 'Add a required confirmation parameter (e.g., "confirm": true) for destructive operations. Remove skip/bypass flags.',
      });
    }
   ...
   */
  private detectMissingAuthentication(toolName: string, args: JsonObject): SecurityFinding[] {
    const findings: SecurityFinding[] = [];

    const sensitiveKeywords = ['admin', 'delete', 'update', 'create', 'modify', 'configure'];
    const isSensitiveTool = sensitiveKeywords.some(kw => toolName.toLowerCase().includes(kw));

    if (!isSensitiveTool) return findings;

    // Check if there's an auth/token/credential parameter
    const hasAuth = Object.keys(args).some(key =>
    ...
    if (!hasAuth) {
      findings.push({
        ...
        remediation: 'Add authentication parameters (e.g., "authToken", "apiKey") for sensitive operations.',
      });
    }
  ...
  private isSuspiciousTool(message: JsonRpcMessage): boolean {
    ...
    const suspiciousKeywords = [
      'execute', 'exec', 'run', 'eval', 'delete', 'destroy', 'drop',
      'admin', 'root', 'sudo', 'shell', 'bash', 'cmd', 'powershell',
      'install', 'download', 'upload', 'write', 'create', 'modify',
  ...
  private requiresDeepAnalysis(message: JsonRpcMessage): boolean {
    ...
    const deepAnalysisKeywords = [
      'agent', 'swarm', 'coordinate', 'orchestrat', 'plugin', 'load',
      'inject', 'poison', 'spoof', 'hijack', 'backdoor', 'exfiltrat',
    ];

    return deepAnalysisKeywords.some(kw => toolName.toLowerCase().includes(kw));
  }
