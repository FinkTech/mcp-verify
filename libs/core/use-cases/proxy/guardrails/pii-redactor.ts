...
import type { JsonValue, JsonObject } from '../../../domain/shared/common.types';
...
export class PIIRedactor implements IGuardrail {
  ...
  private patterns = {
    ...
    genericToken: {
      pattern: /(token|secret|password|passwd|pwd)\s*[=:]\s*([A-Za-z0-9+/=\-_]{8,})/gi,
   ...
   */
  private redactMessage(message: JsonValue, type: 'request' | 'response'): {
    message: JsonValue;
    wasRedacted: boolean;
    ...
    try {
      ...
      return { message, wasRedacted: false, hadCriticalPII: false, detectedTypes: [] };
    }
