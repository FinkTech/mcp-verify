...
export class MockServer {
    ...
    start(): Promise<void> {
        return new Promise((resolve) => {
            ...
            this.server.listen(this.port, () => {
                this.logger.info(`${t('mock_server_running')} http://localhost:${this.port}`);
                resolve();
            });
        });
    }
    ...
    private handleHttpPost(req: http.IncomingMessage, res: http.ServerResponse) {
        ...
        req.on('end', () => {
            try {
                ...
                this.logger.error(t('error_handling_request'), error instanceof Error ? error : undefined);
    ...
    private handleSseGet(req: http.IncomingMessage, res: http.ServerResponse) {
        ...
        if (!messageParam) {
            res.writeHead(400);
            res.end('Missing message parameter (SSE transport expects ?message=JSON)');
            return;
        }
    ...
    private processRpc(jsonRpc: Record<string, unknown>): Record<string, unknown> {
        switch (jsonRpc.method) {
            ...
            case 'tools/call':
                const params = jsonRpc.params as { name: string; arguments: Record<string, any> };
            ...
            case 'resources/list':
                return {
                    resources: [
                        { name: 'app-logs', uri: 'file:///var/log/app.log', mimeType: 'text/plain' }
                    ]
                };
            case 'prompts/list':
                return {
                    prompts: [
                        { name: 'debug-error', description: 'Analyze an error log', arguments: [] },
                        { name: 'write-code', description: 'Generate code snippet', arguments: [{ name: 'language', required: true }] }
                    ]
                };
            case 'prompts/get':
                ...
                return {
                    ...
                    messages: [
                        { role: 'user', content: { type: 'text', text: 'This is a mock prompt for ' + promptParams.name } }
                    ]
                };
            default:
                return {};
