...
const SQL_BLOCKED_KEYWORDS =
  /\b(UNION|SELECT|INSERT|DELETE|UPDATE|DROP|CREATE|ALTER|EXEC|EXECUTE|TRUNCATE|SCHEMA|INFORMATION_SCHEMA)\b/gi;
...
function detectSqlTimingPayload(sql) {
  ...
  const waitMatch = normalised.match(
    /WAITFOR\s+DELAY\s+'(\d{1,2}):(\d{2}):(\d{2})'/i,
  );
  if (waitMatch) {
...
function resolveLogPath(rawPath) {
  ...
  return {
    content: `[INFO] 2025-01-01 00:00:00 Server started\n[INFO] 2025-01-01 00:00:01 Listening on port 3005\n`,
  };
}
...
function sanitiseForTemplate(input) {
  ...
  return input.replace(/<script\b[^>]*>/gi, "").replace(/<\/script>/gi, "");
}
...
const TOOL_SCHEMAS = [
  ...
  },
  {
    name: "read_log",
    description:
      "Read a server log file. Allowed files: app.log, access.log, error.log.",
    inputSchema: {
      type: "object",
      properties: {
        path: {
          type: "string",
          description:
            'Log file path relative to /var/logs/. Example: "app.log"',
        },
      },
      required: ["path"],
    },
  ...
  switch (toolName) {
    case "query_users": {
      ...
      return makeJsonRpcToolResult(null, [
        {
          type: "text",
          text: JSON.stringify({ users: result, count: result.length }),
        },
      ]);
    }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/message") {
    const sessionId =
      url.searchParams.get("sessionId") || req.headers["mcp-session-id"];
...
server.listen(PORT, () => {
  ...
  console.log(
    "    query_users    — Blind SQLi (time-based) behind keyword filter",
  );
  console.log(
    "    ping_host      — Blind Command Injection via URL-decoded newline",
  );
  console.log("    read_log       — Path Traversal via double URL encoding");
  console.log(
    "    render_template — Reflected XSS via JSON structure breakout",
  );
  console.log("");
