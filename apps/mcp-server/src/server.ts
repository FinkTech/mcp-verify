...
const TOOLS: Tool[] = [
  ...
  },
  {
    name: "fuzzTool",
    description:
      "Execute selective fuzzing on a specific MCP tool to identify security vulnerabilities. Supports light, balanced, and aggressive profiles for targeted security testing.",
    inputSchema: {
      type: "object",
      properties: {
        command: {
        ...
        profile: {
          ...
          description:
            "Fuzzing intensity: light (25 payloads, 30s), balanced (50 payloads, 60s), aggressive (100 payloads, 120s)",
          default: "balanced",
        },
        maxDuration: {
          type: "number",
  ...
  },
  {
    ...
    description:
      "Analyze an MCP tool for malicious intent using strict LLM analysis. Detects discrepancies between tool descriptions and actual capabilities to identify deceptive or dangerous tools.",
    inputSchema: {
      type: "object",
      properties: {
        command: {
          type: "string",
          description:
            "Command to run the MCP server (if fetching tool from server)",
        },
        args: {
          type: "array",
          items: { type: "string" },
        ...
        toolDefinition: {
          type: "object",
          description:
            "Direct tool definition to analyze (alternative to fetching from server)",
        },
        llmProvider: {
          type: "string",
  ...
  },
  {
    ...
    description:
      "Analyze MCP tool input schema and suggest security-hardened version with constraints (maxLength, patterns, bounds, enums). Implements the Shield Pattern for automatic schema hardening.",
    inputSchema: {
      type: "object",
      properties: {
        command: {
        ...
        strictness: {
          ...
          description:
            "Hardening strictness level: minimal (DoS only), balanced (recommended), maximum (strict constraints)",
          default: "balanced",
        },
      },
      required: [],
