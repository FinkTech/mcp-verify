 ...
 * Generate human-readable summary
 */
function generateLLMSummary(report: Report, recommendation: string): string {
  const parts: string[] = [];

  // Overall status
  if (recommendation === "blocking_issues") {
    parts.push(
      `🔴 CRITICAL: This MCP server has blocking security issues that must be fixed before deployment.`,
    );
  } else if (recommendation === "review_required") {
    parts.push(
      `🟡 WARNING: This MCP server has security concerns that should be reviewed.`,
    );
  } else {
    parts.push(
      `🟢 This MCP server passes security validation and appears safe to deploy.`,
    );
  }
  ...
  if (report.protocolCompliance && !report.protocolCompliance.passed) {
    parts.push(
      `Protocol compliance issues detected - server may not follow JSON-RPC 2.0 standards correctly.`,
    );
  }
 ...
 * Generate actionable next steps
 */
function generateNextSteps(report: Report, recommendation: string): string[] {
  const steps: string[] = [];

  const criticalFindings = report.security.findings.filter(
    (f) => f.severity === "critical",
  );
  const highFindings = report.security.findings.filter(
    (f) => f.severity === "high",
  );
  ...
  criticalFindings.slice(0, 3).forEach((finding, i) => {
    steps.push(
      `Fix CRITICAL issue #${i + 1}: ${finding.message} (${finding.ruleCode || finding.component})`,
    );
  });
  ...
  if (criticalFindings.length === 0 && highFindings.length > 0) {
    highFindings.slice(0, 2).forEach((finding, i) => {
      steps.push(
        `Fix HIGH severity issue #${i + 1}: ${finding.message} (${finding.ruleCode || finding.component})`,
      );
    });
  }
  ...
  if (report.quality && report.quality.issues.length > 0) {
    ...
    steps.push(
      `Improve quality: ${qualityIssue.message} - ${qualityIssue.suggestion}`,
    );
  }
  ...
  if (
    ...
    steps.push(
      `Fix protocol issue: ${protocolIssue.message} (${protocolIssue.code})`,
    );
  }
  ...
  if (steps.length > 0) {
    steps.push(
      `After fixes, re-run validation to verify: validateServer({command: "node server.js"})`,
    );
  }
  ...
  if (steps.length === 0) {
    steps.push(
      `Consider running stress testing: stressTest({command: "node server.js", users: 10})`,
    );
    steps.push(
      `Consider enabling fuzzing for chaos testing: runFuzzing({command: "node server.js"})`,
    );
  }
...
function determineImpact(finding: SecurityFinding): string {
  ...
  if (ruleCode.includes("001"))
    return "SQL injection attack - database compromise";
  if (ruleCode.includes("002"))
    return "Command injection - arbitrary code execution";
  if (ruleCode.includes("003")) return "SSRF attack - internal network access";
  if (ruleCode.includes("004"))
    return "Data leakage - sensitive information exposure";
  if (ruleCode.includes("005"))
    return "Path traversal - unauthorized file access";
  if (ruleCode.includes("006")) return "XSS attack - user session hijacking";
  if (ruleCode.includes("007"))
    return "XXE attack - server-side request forgery";
  if (ruleCode.includes("008"))
    return "Deserialization attack - remote code execution";
  if (ruleCode.includes("009"))
    return "Weak authentication - unauthorized access";
  if (ruleCode.includes("010"))
    return "Sensitive data exposure - privacy breach";
  if (ruleCode.includes("011")) return "DoS attack - service unavailability";
  if (ruleCode.includes("012"))
    return "Missing security headers - various attacks";
