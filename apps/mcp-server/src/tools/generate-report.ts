...
function formatTextReport(report: Report): string {
  ...
  lines.push('-'.repeat(80));
  lines.push(`Tools: ${report.tools.count} (${report.tools.valid} valid, ${report.tools.invalid} invalid)`);
  lines.push(`Resources: ${report.resources.count} (${report.resources.valid} valid, ${report.resources.invalid} invalid)`);
  lines.push(`Prompts: ${report.prompts.count} (${report.prompts.valid} valid, ${report.prompts.invalid} invalid)`);
  lines.push('');
  ...
  if (report.security.findings && report.security.findings.length > 0) {
    lines.push('Findings:');
    (report.security.findings as SecurityFinding[]).forEach((f: SecurityFinding, i: number) => {
  ...
  if (report.quality.issues && report.quality.issues.length > 0) {
    lines.push('Issues:');
    (report.quality.issues as QualityIssue[]).forEach((i: QualityIssue, idx: number) => {
  ...
  if (report.protocolCompliance) {
    ...
    lines.push('-'.repeat(80));
    lines.push(`Tests Passed: ${report.protocolCompliance.testsPassed ?? 0}/${report.protocolCompliance.totalTests ?? 0}`);
