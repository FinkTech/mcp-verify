...
function buildSummary(
  ...
  if (shadowed.length > 0) {
    lines.push(
      `   • ${shadowed.length} SHADOWED (overridden by a more-local config — these are ignored)`,
    );
  }
  ...
  if (shadowed.length > 0) {
    ...
    for (const s of shadowed) {
      lines.push(
        `  ⚠️  "${s.name}"  [${s.sourceLabel}] — SHADOWED by a more-local config`,
      );
    }
  }
...
function buildNextSteps(servers: InstalledServer[]): string[] {
  ...
  const steps: string[] = [`Run a full health check: selfAudit()`];
            ...
            text: JSON.stringify(
              {
                ...
                serverNames: [],
                llm_summary:
                  "📭 No MCP configuration files found in any standard location.",
                next_steps: [
              ...
              } satisfies ListInstalledServersOutput,
              null,
              2,
            ),
          },
