 ...
 */
export async function fuzzToolTool(
  args: unknown
): Promise<FuzzToolResult> {
  const {
    command,
    args: serverArgs = [],
    toolName,
  ...
  try {
    ...
    const response = {
      ...
      recommendation: criticalVulns.length > 0
        ? 'blocking_issues'
        : highVulns.length > 0
        ? 'review_required'
        : 'safe',

      llm_summary: `🎯 Fuzzing completed for tool "${toolName}" in ${executionTime}s. ` +
      ...
      fuzzing_stats: {
        ...
        mutations_used: fuzzConfig.useMutations ? fuzzConfig.mutationsPerPayload : 0,
      ...
      next_steps: allFindings.length > 0 ? [
        ...criticalVulns.map(v =>
          `FIX CRITICAL: ${v.description} - ${v.remediation}`
        ),
        ...highVulns.slice(0, 3).map(v =>
          `FIX HIGH: ${v.description} - ${v.remediation}`
        ),
