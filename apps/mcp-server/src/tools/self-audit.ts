...
import { createScopedLogger, translations, Language, DiagnosticRunner } from '@mcp-verify/core';
import { NodeRuntimeCheck, GitInstallationCheck, PythonRuntimeCheck, DenoRuntimeCheck } from '@mcp-verify/core/infrastructure/diagnostics/checks/environment-checks';
import { MCPValidator } from '@mcp-verify/core/use-cases/validator/validator';
import { discoverConfigs, RawServerEntry, ConfigSource } from '../utils/config-discovery.js';
...
type CommandFormatIssue = 'ARGS_IN_COMMAND' | 'BINARY_NOT_FOUND' | 'ARG0_NOT_FOUND';
...
interface EnvironmentHealthReport {
  ...
  configs: {
    found: number;
    sources: Array<{ label: string; path: string; serverCount: number; parseError?: string }>;
  };
  servers: {
    total: number;
    healthy: number;
...
function detectArgsMixedInCommand(command: string): boolean {
  ...
  return /\s+[A-Za-z]:[/\\]/.test(command) ||   // Windows absolute path in string
         /\s+\/[a-zA-Z]/.test(command)    ||     // Unix absolute path in string
         /\s+-/.test(command)             ||     // flags mixed in
         /\s+\w.*\.js/.test(command);            // .js file mixed in
}
...
function validateCommandDna(entry: RawServerEntry): CommandDnaResult {
  ...
  if (detectArgsMixedInCommand(entry.command)) {
    ...
    result.suggestedFix =
      `"command": "${suggestedCommand}", "args": [${suggestedArgs.map(a => `"${a}"`).join(', ')}]`;
  }
  ...
  const scriptArg = args.find(a => !a.startsWith('-') && (a.endsWith('.js') || a.endsWith('.ts') || a.endsWith('.mjs') || a.endsWith('.cjs')));
...
function isProcessRunning(command: string, dna: CommandDnaResult): boolean {
  try {
    ...
    if (os.platform() === 'win32') {
      const output = execSync(`tasklist /FI "IMAGENAME eq ${searchName}.exe" /NH`, {
        encoding: 'utf8',
...
async function testServerLife(
...
): Promise<{ health: ServerHealthStatus; detail?: string; latencyMs?: number }> {

  // If command DNA is broken, skip the network test — it will never connect
  if (!dna.binaryFound || (!dna.arg0Exists && dna.resolvedArg0Path !== undefined)) {
    return {
      health: 'MISCONFIGURED',
      detail: dna.formatIssue === 'ARGS_IN_COMMAND'
  ...
  try {
    ...
    const spawnCmd = (os.platform() === 'win32' && entry.command.includes(' ') && !entry.command.startsWith('"'))
      ? `"${entry.command}"`
      : entry.command;
    ...
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error('Handshake timeout')), HANDSHAKE_TIMEOUT_MS)
    );
    ...
    if (latencyMs >= HANDSHAKE_TIMEOUT_MS && processRunning) {
      return { health: 'ZOMBIE', detail: `Process exists but did not respond within ${HANDSHAKE_TIMEOUT_MS/1000}s` };
    }
    ...
    try { validator?.cleanup(); } catch { /* ignore cleanup errors */ }
  }
}
...
function buildLlmSummary(
  ...
  for (const src of sources) {
    if (src.parseError) {
      ...
      lines.push(`  📄 ${src.label} (${Object.keys(src.servers).length} server(s))`);
      lines.push(`     Path: ${src.path}`);
    }
  }
  ...
  if (shadowed.length > 0) {
    ...
    for (const s of shadowed) {
      lines.push(`     • ${s.name}  [${s.sourceLabel}] is overridden — its settings are ignored`);
    }
  }
  ...
  if (misconfigured.length > 0 || zombies.length > 0) {
    lines.push('⚡ Action required: Fix misconfigured servers before the agent can use them.');
  } else if (ghosts.length > 0) {
    lines.push('ℹ️  Some servers are configured but not running. Start them to make them available.');
...
export async function selfAuditTool(args: unknown): Promise<SelfAuditResult> {
  ...
  logger.info('Starting selfAudit v2.1', { metadata: { configPath, skipServerValidation } });
  ...
  try {
    ...
    const envStatus: 'healthy' | 'issues_detected' =
      diagResults.every(r => r.status === 'pass' || r.status === 'skip')
        ? 'healthy'
        : 'issues_detected';
    ...
    logger.info(`Discovered ${sources.length} config file(s), ${resolved.length} server entries`);
    ...
    const report: EnvironmentHealthReport = {
      ...
      environment: {
        status: envStatus,
        checks: diagResults.map(r => ({ name: r.name, status: r.status, message: r.message ?? '' })),
      },
      configs: {
      ...
      servers: {
        ...
        misconfigured: serverEntries.filter(e => e.health === 'MISCONFIGURED').length,
