...
export async function compareServersTool(
          ...
          text: JSON.stringify(
            {
              ...
              llm_summary:
                "⚠️ Cannot compare servers: At least 2 servers are required.",
              next_steps: [
  ...
  try {
    ...
    for (const server of finalServers) {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            `Failed to parse validation result for ${server.name}: ${(parseError as Error).message}`,
          );
        }
    ...
    let reportPath = "";
    try {
      // Map local ValidationResult to the shared ComparisonResult type for reporting
      const reportResults = validationResults.map((r) => ({
        ...r,
        status: (r.status === "failed" ? "error" : "validated") as
          | "error"
          | "validated",
      }));
      ...
      const savedReports = await ReportingService.saveReport(
        {
          kind: "compare",
          data: { results: reportResults, analysis },
        },
        {
          outputDir: "./reports",
      ...
      logger.info("Comparison report saved", {
        metadata: { path: reportPath as any },
      });
    } catch (err) {
          ...
          text: JSON.stringify(
            {
              ...
              comparison: {
                ...
                validatedServers: validationResults.filter(
                  (r) => r.status === "validated",
                ).length,
                failedValidations: validationResults.filter(
                  (r) => r.status === "failed",
                ).length,
              ...
              // Side-by-side scores
              scoreComparison: {
                security: validationResults.map((r) => ({
                  name: r.name,
                  score: r.scores.security,
                })),
                quality: validationResults.map((r) => ({
                  name: r.name,
                  score: r.scores.quality,
                })),
                overall: validationResults.map((r) => ({
                  name: r.name,
...
function analyzeComparison(results: ValidationResult[]): ComparisonAnalysis {
  ...
  const avgSecurity = Math.round(
    validResults.reduce((sum, r) => sum + r.scores.security, 0) /
      validResults.length,
  );

  const avgQuality = Math.round(
    validResults.reduce((sum, r) => sum + r.scores.quality, 0) /
      validResults.length,
  );
  ...
  const avgProtocol = Math.round(
    validResults.reduce((sum, r) => sum + r.scores.protocol, 0) /
      validResults.length,
  );

  const totalFindings = results.reduce(
    (sum, r) => sum + (r.findings?.total || 0),
    0,
  );
...
function generateComparisonSummary(
  ...
  parts.push(
    `📊 SECURITY COMPARISON: Analyzed ${results.length} MCP server(s)`,
  );

  if (failedCount > 0) {
    parts.push(
      `⚠️ ${failedCount} server(s) failed validation and could not be compared.`,
    );
  }
  ...
  if (analysis.mostSecure && analysis.leastSecure) {
    const securityGap =
      analysis.mostSecure.scores.security -
      analysis.leastSecure.scores.security;
    ...
    parts.push(
      `  🔻 Least Secure: "${analysis.leastSecure.name}" (${analysis.leastSecure.scores.security}/100)`,
    );

    if (securityGap > 20) {
      parts.push(
        `  ⚠️ Large security gap detected: ${securityGap} points difference`,
      );
    }
  }
  ...
  if (analysis.highestQuality && analysis.lowestQuality) {
    ...
    parts.push(
      `  🥇 Best Quality: "${analysis.highestQuality.name}" (${analysis.highestQuality.scores.quality}/100)`,
    );
    parts.push(
      `  🔻 Lowest Quality: "${analysis.lowestQuality.name}" (${analysis.lowestQuality.scores.quality}/100)`,
    );
  }
 ...
 */
function generateComparisonNextSteps(
  results: ValidationResult[],
  analysis: ComparisonAnalysis,
): string[] {
  const steps: string[] = [];
  ...
  if (failed.length > 0) {
    failed.forEach((server) => {
      steps.push(
        `Fix validation failure for "${server.name}": ${server.error}`,
      );
    });
  }
  ...
  // Least secure server
  if (
    analysis.leastSecure &&
    analysis.leastSecure.scores.security < 70 &&
    analysis.leastSecure.status === "validated"
  ) {
    steps.push(
      `Priority: Improve security of "${analysis.leastSecure.name}" (currently ${analysis.leastSecure.scores.security}/100)`,
    );
  }

  // Servers with blocking issues
  const blocking = results.filter(
    (r) => r.recommendation === "blocking_issues",
  );
  blocking.forEach((server) => {
    steps.push(
      `URGENT: Fix critical issues in "${server.name}" before deployment`,
    );
  });

  // Servers needing review
  const needsReview = results.filter(
    (r) => r.recommendation === "review_required",
  );
  if (needsReview.length > 0) {
    steps.push(
      `Review security warnings for ${needsReview.length} server(s): ${needsReview.map((s) => s.name).join(", ")}`,
    );
  }

  // Consider standardization
  if (
    analysis.avgSecurity < 80 &&
  ...
  if (steps.length === 0 && results.length > 0) {
    steps.push(
      "All servers are performing well - maintain current security standards",
    );
    steps.push(
      `Consider using "${analysis.mostSecure?.name}" as a security template for other servers`,
    );
  }
