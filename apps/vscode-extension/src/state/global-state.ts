...
class McpVerifyState {
    ...
    private _onActiveServerChanged = new vscode.EventEmitter<ServerInfo | undefined>();
    ...
    private loadPersistedState(): void {
        ...
        // Load recent servers
        const servers = this._context.workspaceState.get<ServerInfo[]>('mcp-verify.servers', []);
        servers.forEach(s => this._servers.set(s.id, s));

        // Load scan history
        const history = this._context.workspaceState.get<ScanResult[]>('mcp-verify.history', []);
    ...
    private persistState(): void {
        ...
        this._context.workspaceState.update('mcp-verify.servers', Array.from(this._servers.values()));
        this._context.workspaceState.update('mcp-verify.history', this._history.slice(0, 50)); // Keep last 50
    }
    ...
    getLatestResult(): ScanResult | undefined {
        ...
        return results.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime())[0];
    }
