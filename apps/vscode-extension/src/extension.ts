...
export function activate(context: vscode.ExtensionContext): void {
    try {
        ...
        context.subscriptions.push(
            vscode.window.registerTreeDataProvider('mcp-verify.servers', serversTreeProvider),
            vscode.window.registerTreeDataProvider('mcp-verify.results', resultsTreeProvider),
            vscode.window.registerTreeDataProvider('mcp-verify.tools', toolsTreeProvider),
            vscode.window.registerTreeDataProvider('mcp-verify.history', historyTreeProvider)
        );
        ...
        // Compare Baseline
        context.subscriptions.push(
            vscode.commands.registerCommand('mcp-verify.compareBaseline', async () => {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('mcp-verify.showFindingDetails', (finding) =>
                runShowFindingDetailsCommand(finding)
            )
        );

        // Show History Report
        context.subscriptions.push(
            vscode.commands.registerCommand('mcp-verify.showHistoryReport', (result) =>
                runShowHistoryReportCommand(context, result)
            )
        );

        // Execute Tool (from tools tree)
        context.subscriptions.push(
            vscode.commands.registerCommand('mcp-verify.executeTool', async (tool) => {
                vscode.window.showInformationMessage(
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand(
                ...
                async (document: vscode.TextDocument, diagnostic: vscode.Diagnostic) => {
                    const { generateSuggestionFile } = await import('./providers/code-actions');
        ...
        vscode.window.showInformationMessage(
            `MCP Verify: ${t('welcome_title')}`
        );

    } catch (error) {
        console.error('Failed to activate MCP Verify:', error);
        if (error instanceof Error) {
            vscode.window.showErrorMessage(`MCP Verify activation failed: ${error.message}`);
        }
    }
}
