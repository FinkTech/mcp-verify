...
import { globalState, ServerInfo, ScanResult, ToolInfo } from '../state/global-state';
...
async function promptForServerCommand(): Promise<{ command: string; executable: string; args: string[] } | undefined> {
    const config = vscode.workspace.getConfiguration('mcpVerify');
    ...
    if (recentCommands.length > 0) {
        const items = [
            ...
            { label: '', kind: vscode.QuickPickItemKind.Separator, command: null as string | null },
...
export async function runValidateCommand(
    ...
    const enableSemanticCheck = config.get<boolean>('validation.enableSemanticCheck', false);
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            const serverId = `${cmdInfo.executable}-${cmdInfo.args.join('-')}`.replace(/[^a-zA-Z0-9-]/g, '_');
            const existingServer = globalState.getServers().find(s => s.command === cmdInfo.executable);
            ...
            logger.info(`Found ${discovery.tools?.length || 0} tools, ${discovery.resources?.length || 0} resources`);
            ...
            if (report.security?.findings) {
                const vulnTools = new Set(report.security.findings.map(f => f.toolName).filter(Boolean));
            ...
            // Show report panel
            McpReportPanel.createOrShow(context.extensionUri, report, getLanguage());
...
export async function runFuzzCommand(
    ...
    const enableFingerprinting = config.get<boolean>('fuzzing.enableFingerprinting', false);
        ...
        try {
            ...
            const engine = new FuzzerEngine({
                ...
                onVulnerability: (detection, payload) => {
                    ...
                    logger.warn(`Vulnerability found: ${detection.vulnerabilityType} (${detection.severity})`);
                },
                onFingerprint: (fingerprint) => {
                    logger.info(`Server fingerprint: ${fingerprint.language}/${fingerprint.framework}`);
                }
            });
            ...
            const discoveredTools = (listToolsResponse as { tools?: Array<{ name: string }> }).tools || [];
            ...
            // Run fuzzing session
            progress.report({ message: `Running fuzzing engine on ${selectedTool || 'all tools'}...` });
            const session = await engine.fuzz(fuzzTarget, selectedTool || 'all', undefined);

            // Close transport
            transport.close();

            // Show results
            logger.success(`Fuzzing complete: ${session.payloadsExecuted} payloads executed, ${session.vulnerabilities.length} vulnerabilities found`);
            ...
            if (session.vulnerabilities.length === 0) {
                ...
                const findings: SecurityFinding[] = session.vulnerabilities.map(vuln => {
                    // Convert evidence to Record<string, JsonValue>
                    const evidence: Record<string, JsonValue> | undefined = vuln.evidence
                        ? {
                ...
                const report: Report = {
                    ...
                    security: {
                        score,
                        level: score >= 90 ? 'safe' : score >= 70 ? 'moderate' : 'critical',
                        findings,
            ...
            if (transport) {
                try {
                    ...
                    logger.warn(`Error closing transport: ${closeError instanceof Error ? closeError.message : String(closeError)}`);
                }
            }
        }
    });
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            while (Date.now() < endTime && !token.isCancellationRequested) {
                ...
                progress.report({ message: `${remaining}s remaining... (${requestCount} requests)` });
            }

            const totalTime = (Date.now() - startTime) / 1000;
            const rps = requestCount / totalTime;

            logger.success(`Stress test complete: ${requestCount} requests, ${errorCount} errors, ${rps.toFixed(2)} RPS`);
...
export async function runDoctorCommand(
    ...
    await vscode.window.withProgress({
        ...
        // Show results
        const outputChannel = vscode.window.createOutputChannel('MCP Verify Diagnostics');
        outputChannel.clear();
        ...
        if (issues.length === 0) {
            vscode.window.showInformationMessage('MCP Verify: All diagnostics passed!');
        } else {
            vscode.window.showWarningMessage(`MCP Verify: ${issues.length} issue(s) found. Check output for details.`);
        }
    });
}
...
export async function runExportSarifCommand(
    ...
    if (!result) {
        vscode.window.showWarningMessage('No scan results to export. Run a validation first.');
        return;
    }

    const uri = await vscode.window.showSaveDialog({
        defaultUri: vscode.Uri.file(`mcp-verify-${result.serverName}-${Date.now()}.sarif`),
        filters: {
            'SARIF': ['sarif', 'json']
        }
    });
    ...
    try {
        ...
        vscode.window.showInformationMessage(`SARIF report exported: ${uri.fsPath}`);
    } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
export async function runGenerateBadgeCommand(
    ...
    if (!result) {
        vscode.window.showWarningMessage('No scan results. Run a validation first.');
        return;
    }
    ...
    try {
        ...
        vscode.window.showInformationMessage(`Security badge exported: ${uri.fsPath}`);
    } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
export async function runSuggestSecureSchemaCommand(
    ...
    try {
        ...
        if (!editor) {
            vscode.window.showWarningMessage('MCP Verify: No active editor found. Please open a file containing a tool schema.');
            return;
        }
        ...
        try {
            ...
            vscode.window.showErrorMessage('MCP Verify: Active file is not valid JSON. Please open a tool schema file.');
            return;
        }

        // Validate it looks like a tool schema
        if (!toolSchema || typeof toolSchema !== 'object') {
            vscode.window.showErrorMessage('MCP Verify: Active file does not contain a valid tool schema object.');
            return;
        }
        ...
        vscode.window.showErrorMessage(`MCP Verify: Failed to generate secure schema - ${message}`);
    }
}
...
function applyShieldPattern(schema: unknown): unknown {
    ...
    if ('inputSchema' in result && result.inputSchema && typeof result.inputSchema === 'object') {
 ...
 */
function hardenInputSchema(inputSchema: Record<string, unknown>): Record<string, unknown> {
    ...
    // Process properties
    if ('properties' in hardened && hardened.properties && typeof hardened.properties === 'object') {
    ...
    // Process array items
    if ('items' in hardened && hardened.items && typeof hardened.items === 'object') {
        hardened.items = hardenInputSchema(hardened.items as Record<string, unknown>);
    }
 ...
 */
function hardenProperty(name: string, prop: Record<string, unknown>): Record<string, unknown> {
    const hardened = { ...prop };
    ...
    const isCritical = CRITICAL_PARAMS.some(kw => name.toLowerCase().includes(kw));
    ...
    if (propType === 'string') {
        ...
        if (isCritical && !hardened.pattern && !hardened.enum) {
            ...
            } else if (name.toLowerCase().includes('path') || name.toLowerCase().includes('filename')) {
    ...
    if (propType === 'number' || propType === 'integer') {
        ...
        if (hardened.minimum === undefined && hardened.exclusiveMinimum === undefined) {
            hardened.minimum = 0; // Safe default for most numeric inputs
        }

        // Add maximum if missing
        if (hardened.maximum === undefined && hardened.exclusiveMaximum === undefined) {
            hardened.maximum = propType === 'integer' ? Number.MAX_SAFE_INTEGER : Number.MAX_VALUE;
        }
    }
...
export function runShowHistoryReportCommand(
    ...
    McpReportPanel.createOrShow(context.extensionUri, result.report, getLanguage());
}
