...
import { initLanguage, getCurrentLanguage, t, detectLanguage, setLanguage } from '@mcp-verify/shared';
...
const wrapAction = <T extends (...args: any[]) => Promise<number | void>>(action: T) => {
...
program
  ...
  .version(packageJson.version, '-v, --version', 'Show version with Yogui mascot')
...
program
  ...
  .option('--rules <list>', 'Comma-separated list of rule blocks to execute (e.g., OWASP,MCP,A)')
  .option('--exclude-rules <list>', 'Comma-separated list of rule blocks or IDs to exclude (e.g., SEC-001,Weaponization)')
  .option('--min-severity <level>', 'Minimum severity level to report (info, low, medium, high, critical)')
  ...
  .option('--allowed-score-drop <number>', t('option_allowed_score_drop_desc'), '5')
...
program
  ...
  .option('--type <type>', 'Specific disclaimer type (fuzz, stress, proxy, validate)')
...
const hasCommand = process.argv.slice(2).some(arg =>
  !arg.startsWith('-') && ['validate', 'stress', 'mock', 'init', 'dashboard', 'play', 'proxy', 'doctor', 'examples', 'fuzz', 'disclaimers', 'scan-config'].includes(arg)
);
...
`));
  ...
  console.log(chalk.dim(`  📧 ${t('email_label').padEnd(10)}: `) + chalk.cyan('hello.finksystems@gmail.com'));
  console.log(chalk.dim(`  🌐 ${t('github_label').trim().padEnd(10)}: `) + chalk.cyan('github.com/FinkTech'));
  console.log(chalk.dim(`  💼 ${t('linkedin_label').trim().padEnd(10)}: `) + chalk.cyan('linkedin.com/in/ariel-fink'));

  console.log(chalk.yellow(`\n  ⚠️  Disclaimer: ${t('cli_disclaimer_independent') || 'This is an independent open-source tool.'}`));
  console.log(chalk.gray(`      ${t('cli_disclaimer_affiliation') || 'Not affiliated with Anthropic or the Model Context Protocol organization.'}`));
