...
export function displayReportSummary(
  ...
  if (report.status === "valid") {
    console.log(
      chalk.bgGreen.white.bold(`  ✅ ${t("status_valid").toUpperCase()}  `),
    );
  } else {
    console.log(
      chalk.bgRed.white.bold(`  ❌ ${t("status_invalid").toUpperCase()}  `),
    );
  }
  ...
  if (report.fuzzing) {
    ...
    if (report.fuzzing.crashes > 0) {
      ...
      console.log(
        chalk.yellow(`  ${t("failures_label")}: ${report.fuzzing.failedTests}`),
      );
    } else {
  ...
  if (report.security.findings.length > 0) {
    report.security.findings.forEach((finding) => {
      ...
      console.log(
        `  • [${severityColor(finding.severity.toUpperCase())}] ${finding.message}`,
      );
    });
  }
  ...
  if (htmlReportPath)
    console.log(`${t("html_label")}: ${chalk.blue(htmlReportPath)}`);
  console.log("");
}
...
export function displayValidationError(error: unknown, target: string): void {
  ...
  if (
    ...
    console.log(
      chalk.dim("  " + t("try_prefix") + " " + t("tip_check_process")),
    );
    console.log(
    ...
    console.log(
      chalk.dim("  " + t("try_prefix") + " " + t("tip_use_verbose_raw") + "\n"),
    );
  } else {
    console.log(chalk.gray("• ") + t("unexpected_error"));
    console.log(chalk.dim("  " + t("try_prefix") + " " + t("tip_verbose")));
    console.log(
      chalk.dim("  " + t("try_prefix") + " mcp-verify doctor " + target + "\n"),
    );
  }
