...
export class McpFuzzTarget implements FuzzTarget {
  ...
  async discoverParam(): Promise<DiscoveryResult> {
    ...
    try {
      ...
      const stringParam = propEntries.find(
...
function printVulnerability(detection: DetectionResult, index: number): void {
  ...
  console.log(
    `      ${chalk.dim("Severity:")} ${formatSeverity(detection.severity)} | ${chalk.dim("Confidence:")} ${detection.confidence}`,
  );
  ...
  if (detection.remediation) {
    console.log(
      `      ${chalk.dim("Fix:")} ${detection.remediation.substring(0, 100)}...`,
    );
  }
}
...
function printSessionSummary(session: FuzzingSession): void {
  ...
  console.log(
    `  ${chalk.dim("Payloads:")} ${session.payloadsExecuted}/${session.totalPayloads}`,
  );
  ...
  if (session.feedbackStats) {
    ...
    console.log(
      `    ${chalk.dim("Interesting responses:")} ${fb.interestingResponsesFound}`,
    );
    console.log(
      `    ${chalk.dim("Mutations injected:")}    ${fb.mutationsInjected}`,
    );
    console.log(
      `    ${chalk.dim("Mutation rounds:")}       ${fb.mutationRoundsCompleted}`,
    );

    if (fb.timingAnomaliesDetected > 0) {
      console.log(
        `    ${chalk.yellow("Timing anomalies:")}       ${fb.timingAnomaliesDetected}`,
      );
    }
    if (fb.structuralDriftDetected > 0) {
      console.log(
        `    ${chalk.yellow("Structural drifts:")}      ${fb.structuralDriftDetected}`,
      );
    }
    if (fb.serverCrashesDetected > 0) {
      console.log(
        `    ${chalk.red("Server crashes:")}         ${fb.serverCrashesDetected}`,
      );
    }
  }
  ...
  if (session.vulnerabilities.length > 0) {
    console.log(
      `  ${chalk.dim("Vulnerabilities:")} ${chalk.red.bold(session.vulnerabilities.length)}`,
    );
  ...
  if (session.errors.length > 0) {
    console.log(
      `  ${chalk.dim("Errors:")} ${chalk.yellow(session.errors.length)}`,
    );
  }
...
function printFingerprintResult(fingerprint: ServerFingerprint): void {
  ...
  const langColor =
    fingerprint.languageConfidence >= 0.8
      ? chalk.green
      : fingerprint.languageConfidence >= 0.5
        ? chalk.yellow
        : chalk.gray;
  console.log(
    `  ${chalk.dim("Language:")} ${langColor.bold(fingerprint.language.toUpperCase())} ${chalk.dim(`(${langConfidence}% confidence)`)}`,
  );
  ...
  if (fingerprint.framework !== "unknown") {
    ...
    console.log(
      `  ${chalk.dim("Framework:")} ${chalk.cyan(fingerprint.framework)} ${chalk.dim(`(${fwConfidence}%)`)}`,
    );
  }
  ...
  if (fingerprint.database !== "unknown" && fingerprint.database !== "none") {
    console.log(
      `  ${chalk.dim("Database:")} ${chalk.magenta(fingerprint.database)}`,
    );
  }
  ...
  if (fingerprint.recommendedGenerators.length > 0) {
    ...
    if (fingerprint.recommendedGenerators.length > 5) {
      console.log(
        `    ${chalk.gray(`... and ${fingerprint.recommendedGenerators.length - 5} more`)}`,
      );
    }
  }
  ...
  console.log("");
  console.log(
    `  ${chalk.dim("Fingerprint time:")} ${fingerprint.durationMs}ms`,
  );
  console.log(chalk.gray("─".repeat(50)));
}
...
async function saveReports(
  ...
  const result = await ReportingService.saveReport(
...
export async function runFuzzAction(
  ...
  const generatorNames = (options.generators || "all")
    .split(",")
  ...
  const generatorMap: Record<string, () => IPayloadGenerator> = {
    ...
    object: () =>
      new ClassicPayloadGenerator({ categories: ["objectInjection"] }),
  ...
  if (generators.length === 0) {
    ...
    console.log(
      chalk.gray(
        "  Classic:    classic (all), sqli, xss, cmd, path, ssrf, xxe, nosql, ssti",
      ),
    );
  ...
  const detectorNames = (options.detectors || "all")
    .split(",")
  ...
  if (detectors.length === 0) {
    spinner.warn(
      t("no_detectors_selected") || "No detectors selected, using all",
    );
    detectors.push(new PromptLeakDetector());
    detectors.push(new JailbreakDetector());
    detectors.push(new ProtocolViolationDetector());
  ...
  if (options.header && options.header.length > 0) {
    ...
    if (Object.keys(headers).length > 0) {
      spinner.info(
        chalk.cyan(
          `Using ${Object.keys(headers).length} custom header(s): ${Object.keys(headers).join(", ")}`,
        ),
      );
      spinner.start();
    }
  ...
  // Create target
  const fuzzTarget = new McpFuzzTarget(
    target,
    transportType as "stdio" | "http",
    {
      tool: options.tool,
      param: options.param,
      timeout,
  ...
  const engine = new FuzzerEngine({
    ...
    onProgress: (progress) => {
      spinner.text =
        `${t("fuzzing_progress")}: ${progress.percentage}% (${progress.current}/${progress.total}) | ` +
  ...
  try {
    ...
    if (discovery.autoDetected) {
      spinner.info(
        chalk.cyan(
          `Auto-detected parameter: ${chalk.bold(discovery.paramName)} (${discovery.paramType})`,
        ),
      );
      if (discovery.availableParams.length > 1) {
        console.log(
          chalk.gray(
            `  Available params: ${discovery.availableParams.join(", ")}`,
          ),
        );
      }
      spinner.start();
      ...
      spinner.warn(
        chalk.yellow(
          `Tool "${options.tool}" not found, using default param: ${discovery.paramName}`,
        ),
      );
      spinner.start();
    }
    ...
    const payloadCount = engine.generatePayloads(
    ...
    try {
      ...
      if (error instanceof Error && error.message === "PANIC_STOP_429") {
        ...
        if (partialSession) {
          console.log(
            chalk.dim(
              `Payloads tested before stop: ${partialSession.totalPayloads}`,
            ),
          );
          console.log(
            chalk.dim(
              `Vulnerabilities found: ${partialSession.vulnerabilities.length}`,
            ),
          );
        }
    ...
    if (session.vulnerabilities.length > 0) {
      spinner.fail(
        chalk.red(
          `${t("fuzz_complete_vulns_found")}: ${session.vulnerabilities.length}`,
        ),
      );
