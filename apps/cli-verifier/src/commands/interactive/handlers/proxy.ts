...
export async function handleProxy(
  ...
  if (positionals.includes('help') || positionals.includes('h') || flags['help']) {
    showProxyHelp();
    return;
  }

  const target = await resolveTarget(args, session, rl, 'proxy "node server.js" --port 9000');
  if (!target) return;
...
function showProxyHelp(): void {
  ...
  console.log(`    ${chalk.cyan('proxy <target>')}           Starts a security gateway between client and server`);
  console.log('');
  console.log(chalk.white('    Options:'));
  console.log(`      ${chalk.yellow('--port <number>')}       Port to listen on (default: 9000)`);
  console.log(`      ${chalk.yellow('--log-file <path>')}     Save session logs to a file`);
  console.log(`      ${chalk.yellow('--timeout <ms>')}        Auto-stop proxy after X milliseconds`);
  console.log('');
  ...
  console.log(chalk.gray('      proxy "node server.js" --port 8080 --log-file audit.log\n'));
}
