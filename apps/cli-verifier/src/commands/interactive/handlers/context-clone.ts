...
export function handleContextClone(
  args: string[],
  ...
  if (!success) {
    ...
    if (!(sourceName in session.state.contexts)) {
      console.log(
        chalk.red(
          `  ✗ ${t("context_clone_source_not_exist", { source: chalk.white(sourceName) })}`,
        ),
      );
      console.log(
        chalk.dim(
          `  ${t("context_clone_available_contexts")}: ${Object.keys(session.state.contexts).join(", ")}`,
        ),
      );
    } else if (targetName in session.state.contexts) {
      console.log(
        chalk.red(
          `  ✗ ${t("context_clone_target_exists", { target: chalk.white(targetName) })}`,
        ),
      );
      ...
      console.log(
        chalk.red(
          `  ✗ ${t("context_clone_failed", { source: sourceName, target: targetName })}`,
        ),
      );
    }
    console.log("");
  ...
  console.log(
    chalk.green(
      `  ✓ ${t("context_clone_success")}: ${chalk.white(sourceName)} → ${chalk.white(targetName)}`,
    ),
  );
  ...
  console.log(
    `    ${chalk.gray(t("context_clone_target_label"))}   ${clonedContext.target ?? chalk.dim("(not set)")}`,
  );
  console.log(
    `    ${chalk.gray(t("context_clone_language_label"))} ${chalk.cyan(clonedContext.lang)}`,
  );
  console.log(
    `    ${chalk.gray(t("context_clone_profile_label"))}  ${chalk.cyan(clonedContext.profile.name)}`,
  );

  if (targetOverride) {
    console.log(
      chalk.yellow(
        `  ⚠️  ${t("context_clone_target_overridden")}: ${chalk.white(targetOverride)}`,
      ),
    );
  }
