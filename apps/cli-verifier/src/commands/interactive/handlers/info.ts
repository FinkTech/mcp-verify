...
export function showHelp(): void {
  ...
  helpRow(
    "history",
    "[--last N] [--clear]",
    t("interactive_show_history_desc"),
  );
  ...
  helpRow(
    "context",
    "list | switch | create | clone | delete",
    t("cmd_context_desc"),
  );
  ...
  console.log(
    chalk.dim("    cmd [args] > file.txt   ") + chalk.gray("overwrite"),
  );
  console.log(chalk.dim("    cmd [args] >> file.txt  ") + chalk.gray("append"));
  console.log(
    chalk.dim("    validate > report.txt   ") +
      chalk.gray("redirect validate output"),
  );

  console.log(chalk.bold.white("\n  Examples:\n"));
  const ex = (s: string) =>
    console.log("    " + chalk.dim("mcp-verify > ") + chalk.cyan(s));
...
export function helpRow(cmd: string, args: string, desc: string): void {
  const C = 20,
    A = 40;
  console.log(
    `    ${chalk.cyan(cmd.padEnd(C))}${chalk.dim(args.padEnd(A))}${desc}`,
  );
}
...
export function showAbout(): void {
  ...
  console.log(
    `  ${chalk.gray("Version:")}        ${chalk.cyan(packageJson.version)}`,
  );
  ...
  [
    t("about_feature_owasp"),
    t("about_feature_llm"),
    t("about_feature_protocol"),
    t("about_feature_reports"),
    t("about_feature_i18n"),
  ].forEach((f) => console.log(`    ${chalk.cyan("✓")} ${chalk.gray(f)}`));
  console.log(chalk.bold.white(`\n  ${t("interactive_links")}:\n`));
  console.log(
    `    ${chalk.gray("GitHub:")}   ${chalk.cyan("github.com/FinkTech/mcp-verify")}`,
  );
  console.log(
    `    ${chalk.gray("Docs:")}     ${chalk.cyan("github.com/FinkTech/mcp-verify#readme")}`,
  );
  console.log(
    `    ${chalk.gray("Security:")} ${chalk.cyan("github.com/FinkTech/mcp-verify/blob/main/SECURITY.md")}`,
  );
  console.log(
    `    ${chalk.gray("Issues:")}   ${chalk.cyan("github.com/FinkTech/mcp-verify/issues")}`,
  );
