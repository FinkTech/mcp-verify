...
export function handleSet(args: string[], session: ShellSession): void {
  ...
  switch (key) {
    case 'target':
      ...
      console.log(chalk.green(`  ✓ ${t('interactive_target')}: ${chalk.white(value)}\n`));
      break;
    ...
    default:
      ...
      console.log(chalk.green(`  ✓ ${t('interactive_config_set')}: ${chalk.white(key)} = ${chalk.white(value)}\n`));
  }
}

export function handleTargetChange(args: string[], session: ShellSession): void {
  if (args.length === 0) {
    const currentTarget = session.state.target ?? chalk.dim(t('interactive_not_set'));
  ...
  if (detection.valid) {
    ...
    console.log(chalk.yellow(`  ⚠️  ${t('interactive_target_set_warning')}: ${chalk.white(value)}`));
    console.log(chalk.yellow(`    ${detection.message}`));
    console.log(chalk.dim(`    ${t('target_validation_will_likely_fail')}\n`));
  }
}

export function handleLanguageChange(args: string[], session: ShellSession): void {
  if (args.length === 0) {
...
function applyLanguageChange(value: string, session: ShellSession): void {
  ...
  console.log(chalk.green(`  ✓ ${t('interactive_lang_success')}: ${chalk.white(lang)}\n`));
}
...
export function handleConfig(session: ShellSession): void {
  ...
  console.log(`  ${chalk.gray(t('interactive_workspace'))}   ${chalk.cyan(workspace ?? chalk.dim(t('interactive_none')))}`);
  console.log(`  ${chalk.gray(t('interactive_language'))}    ${chalk.cyan(lang)}`);

  // SECURITY: Redact target if it contains secrets
  const safeTarget = target ? session.redactSecrets(target) : chalk.dim(t('interactive_not_set'));
  console.log(`  ${chalk.gray(t('interactive_target'))}      ${chalk.cyan(safeTarget)}`);
  ...
  console.log(`  ${chalk.gray(t('interactive_elapsed'))}     ${chalk.cyan(formattedTime)}`);
  console.log(`  ${chalk.gray(t('interactive_started'))}     ${chalk.cyan(startedAt.toLocaleTimeString())}`);
  console.log('');
  const paths = PersistenceManager.getPaths();
  console.log(`  ${chalk.gray(t('interactive_history_file'))} ${chalk.dim(paths.historyFile)}`);
  const sessionExists = fs.existsSync(paths.sessionFile);
  console.log(
    `  ${chalk.gray(t('interactive_session_file'))} ${chalk.dim(paths.sessionFile)} ` +
    (sessionExists ? chalk.green(t('interactive_active')) : chalk.dim(t('interactive_not_saved')))
  );
  console.log('');
}
...
export function showHistory(session: ShellSession, args: string[]): void {
  ...
  const sessionOnly = session.state.history.filter(h => !persisted.includes(h));
  const all         = [...persisted, ...sessionOnly];

  const maxShown = typeof flags['last'] === 'string'
