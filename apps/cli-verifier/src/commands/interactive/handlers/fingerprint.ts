...
export async function handleFingerprint(
  ...
  try {
    ...
    console.log(
      `  Language:  ${result.language === "unknown" ? chalk.gray("Unknown") : chalk.green(result.language)}`,
    );
    console.log(
      `  Framework: ${result.framework === "unknown" ? chalk.gray("Unknown") : chalk.green(result.framework)}`,
    );
    console.log(
      `  Database:  ${result.database === "none" ? chalk.gray("None/Unknown") : chalk.yellow(result.database)}`,
    );
    ...
    console.log(
      chalk.red(
        `  ✗ Fingerprint failed: ${error instanceof Error ? error.message : String(error)}`,
      ),
    );
  } finally {
