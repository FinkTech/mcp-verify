...
function printHeader(session: ShellSession): string[] {
  ...
  function row(
    ...
    return (
      chalk.dim.cyan("│") + content + " ".repeat(rightPad) + chalk.dim.cyan("│")
    );
  }
  ...
  const titleContent =
    chalk.bold.cyan(t("interactive_shell")) +
    chalk.dim("  ·  ") +
    chalk.dim(t("interactive_available_commands"));
  const titleVisible =
    vlen(t("interactive_shell")) +
    5 +
    vlen(t("interactive_available_commands"));
  ...
  lines.push(
    row("interactive_workspace", chalk.white(contextName), contextName),
  );
  lines.push(chalk.dim.cyan(`╰${hbar}╯`));
  lines.push("");
...
function printCommand(cmd: HelpCommand): string[] {
  ...
  const aliases = cmd.aliases.length
    ? chalk.dim("  ·  ") + chalk.dim(cmd.aliases.join("  ·  "))
    : "";

  lines.push(bar + chalk.bold.cyan(cmd.name) + aliases);
  lines.push(
    pipe + chalk.hex("#94a3b8")(t(cmd.descKey as Parameters<typeof t>[0])),
  );

  if (cmd.flags?.length) {
    lines.push(
      pipe + chalk.dim("flags  ") + chalk.hex("#2a5a6a")(cmd.flags.join("  ")),
    );
  }
...
function renderView(session: ShellSession, activeIndex: number): string[] {
  ...
  const tabs = CATEGORY_ORDER.map((cat, idx) => {
    const title = t(CATEGORY_TITLE_KEYS[cat] as Parameters<typeof t>[0])
      .replace(/:$/, "")
      .toUpperCase();
    if (idx === activeIndex) {
  ...
  lines.push(
    chalk.dim("  ←/→ ") +
      chalk.dim(t("interactive_help_change_tab") + "  ·  ") +
      chalk.dim("Esc/Enter/Q ") +
      chalk.dim(t("interactive_help_close")),
  );
  lines.push("");
  ...
  return new Promise((resolve) => {
    ...
    const handler = (key: string) => {
      ...
      if (key === "\x1b[C") {
        ...
        activeIndex =
          (activeIndex - 1 + CATEGORY_ORDER.length) % CATEGORY_ORDER.length;
        redraw();
      }
    };
