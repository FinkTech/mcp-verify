...
export const COMMAND_FLAGS: Readonly<Record<string, string[]>> = {
  validate: [
    '--output', '--format', '--transport', '--html', '--sandbox', '--fuzz',
    '--lang', '--rules', '--exclude-rules', '--min-severity',
    '--llm', '--save', '--verbose',
    '--save-baseline', '--compare-baseline',
    '--fail-on-degradation', '--allowed-score-drop',
  ],
  fuzz: [
    '--tool', '--transport', '--concurrency', '--timeout',
    '--rate-limit', '--param', '--generators', '--detectors',
    '--stop-on-first', '--fingerprint', '--verbose',
    '--output', '--format', '--header',
  ],
  doctor: [
    '--transport', '--watch', '--verbose', '--html', '--md', '--json',
    '--output', '--show-history', '--fix-integrity', '--clean-history',
  ],
...
export const PRIMARY_COMMANDS: readonly string[] = [
  ...
  'proxy', 'mock', 'init', 'fingerprint', 'inspect', 'examples',
  'history', 'config', 'context', 'profile', 'status',
  'about', 'lang', 'set', 'target', 'clear', 'help', 'exit',
];
...
export const ALL_COMMANDS: readonly string[] = [
  ...PRIMARY_COMMANDS,
  'v', 'f', 'd', 's', 'm', 'ex', 'cfg', 'language', 'version',
  'stack', 'ls',
  'github', 'gh', 'linkedin', 'li', 'website', 'web',
  'cls', 'h', 'quit', 'q',
];
...
export class ContextCompleter {
  ...
  static complete(line: string, session?: ShellSession): [string[], string] {
    ...
    if (!trimmed.includes(' ')) {
      const hits = (ALL_COMMANDS as string[]).filter(c => c.startsWith(trimmed));
    ...
    if (partial.startsWith('--') || (startingNewArg && availableFlags.length > 0)) {
    ...
    if (prevToken === '--tool' && session?.state.availableTools) {
      ...
      return [
        completions.length ? completions : session.state.availableTools.slice(0, 10),
        line,
      ];
    }
    ...
    if (looksLikePath || (startingNewArg && ContextCompleter.commandExpectsPath(cmd))) {
  ...
  private static commandExpectsPath(cmd: string): boolean {
    return [
      'validate', 'fuzz', 'doctor', 'stress', 'dashboard',
      'play', 'proxy', 'init', 'fingerprint', 'inspect',
    ].includes(cmd);
  }
