...
const PRIMARY_COMMANDS: readonly string[] = [
  'validate', 'fuzz', 'doctor', 'stress', 'dashboard', 'play',
  'proxy', 'mock', 'init', 'fingerprint', 'inspect', 'examples', 'history', 'config',
  'context', 'profile', 'status',
  'target', 'set', 'lang', 'help', 'about', 'exit',
];
...
export async function dispatch(
  ...
  switch (cmd) {
    // Security tools
    case 'validate': case 'v':    await handleValidate(args, session, rl);  break;
    case 'fuzz':     case 'f':    await handleFuzz(args, session, rl);      break;
    case 'doctor':   case 'd':    await handleDoctor(args, session, rl);    break;
    case 'stress':   case 's':    await handleStress(args, session, rl);    break;
    case 'dashboard':             await handleDashboard(args, session, rl); break;
    case 'play':                  await handlePlay(args, session, rl);      break;
    case 'proxy':                 await handleProxy(args, session, rl);     break;
    case 'mock':     case 'm':    await handleMock(args);                   break;
    case 'init':                  await handleInit(args);                   break;
    case 'fingerprint': case 'stack': await handleFingerprint(args, session, rl); break;
    case 'inspect':     case 'ls':    await handleInspect(args, session, rl);     break;
    case 'examples': case 'ex':   await handleExamples();                   break;

    // Session management
    case 'set':                   handleSet(args, session);                 break;
    case 'target':                handleTargetChange(args, session);        break;
    case 'lang': case 'language': handleLanguageChange(args, session);      break;
    case 'config': case 'cfg':    handleConfig(session);                    break;
    case 'history':               showHistory(session, args);               break;

    // Multi-context workspace
    case 'context':               await handleContextDispatch(args, session); break;
    case 'profile':               handleProfileCommand(args, session);      break;
    case 'status':                await handleStatus(session);              break;

    // Info
    case 'about':                 showAbout();                              break;
    case 'version':               console.log(chalk.cyan(ASCII_ART.version(packageJson.version))); break;
    case 'help': case 'h':        showHelp();                               break;

    // Links
    case 'github':  case 'gh':    openUrl('https://github.com/FinkTech/mcp-verify');  break;
    case 'linkedin': case 'li':   openUrl('https://linkedin.com/in/ariel-fink');      break;
    case 'website': case 'web':   openUrl('https://github.com/FinkTech/mcp-verify'); break;
    ...
    default: {
      console.log(chalk.red(`  ${t('interactive_unknown_command')}: ${chalk.white(cmd)}`));
      ...
      console.log(chalk.dim(`  Type ${chalk.white('help')} to list all commands.\n`));
    }
  }
}
...
async function handleContextDispatch(args: string[], session: ShellSession): Promise<void> {
  if (args.length === 0) {
    showContextHelp();
    return;
  }
...
function showContextHelp(): void {
  ...
  console.log(`    ${chalk.cyan('context list')}                            List all contexts`);
  console.log(`    ${chalk.cyan('context switch <name>')}                   Switch to a context`);
  console.log(`    ${chalk.cyan('context create <name>')}                   Create a new context`);
  console.log(`    ${chalk.cyan('context clone <source> <new_name>')}      Clone an existing context`);
  console.log(`    ${chalk.cyan('context delete <name>')}                   Delete a context`);
  console.log(chalk.dim('\n  Clone examples:'));
  console.log(chalk.dim('    context clone dev staging'));
  console.log(chalk.dim('    context clone dev prod --target "http://prod.example.com"\n'));
}
