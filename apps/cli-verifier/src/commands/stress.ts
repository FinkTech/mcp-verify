...
export async function runStressAction(
  target: string,
  ...
  try {
    ...
    const transportType =
      (options.transport as "http" | "stdio" | undefined) ||
      detectTransportType(target);
    ...
    console.log(
      `${t("total_requests")}: ${chalk.cyan(result.summary.totalRequests)}`,
    );
    console.log(
      `${t("success_rate")}:   ${result.summary.successfulRequests === result.summary.totalRequests ? chalk.green("100%") : chalk.yellow(((result.summary.successfulRequests / result.summary.totalRequests) * 100).toFixed(1) + "%")}`,
    );
    console.log(
      `${t("throughput")}:     ${chalk.bold(result.summary.requestsPerSecond)} ${t("req_sec")}`,
    );
