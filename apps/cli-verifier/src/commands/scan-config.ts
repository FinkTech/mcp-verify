 ...
 */
export async function runScanConfigAction(configPath: string, options: {
  all?: boolean;
  quiet?: boolean;
}): Promise<number> {
  ...
  const spinner = !options.quiet ? ora(`Scanning ${configPath}...`).start() : null;
  ...
  try {
    ...
    console.error(chalk.red(`Error scanning config: ${error instanceof Error ? error.message : String(error)}`));
    return 1;
  }
}
 ...
 */
async function scanAllConfigFiles(rule: MaliciousConfigFileRule, quiet: boolean): Promise<number> {
  ...
  for (const configPath of configPaths) {
    if (!fs.existsSync(configPath)) {
      if (!quiet) {
        console.log(chalk.dim(`  ○ ${path.relative(cwd, configPath)} - not found (skipped)`));
      }
      continue;
    }
    ...
    console.log(chalk.yellow(`  ⚠️  ${path.relative(cwd, configPath)} - ${findings.length} finding(s)`));
