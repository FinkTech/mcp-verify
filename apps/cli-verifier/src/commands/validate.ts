...
import { t, setLanguage, getCurrentLanguage, ReportingService, ReportFormat, captureGitInfo } from '@mcp-verify/shared';
...
import { formatError, createLogger, PathValidator, URLValidator } from '@mcp-verify/shared';
...
export async function runValidationAction(target: string, options: Record<string, unknown>) {
  ...
  try {
    ...
    const transportType = (options.transport as string | undefined) || detectTransportType(target);
    if (spinner) spinner.text = `${t('connecting')} ${String(transportType).toUpperCase()}...`;

    // SECURITY WARNING: Private IP detection
    if (URLValidator.isURL(target) && URLValidator.isPrivateOrLocalhost(target)) {
      ...
      if (spinner) {
        ...
        spinner.start(t('connecting') + ' ' + transportType.toUpperCase() + '...');
      } else if (!isQuiet) {
    ...
    if (llmProvider) {
      ...
      if (spinner) {
        spinner.info(chalk.blue(t('llm_analysis_using', { provider: llmProvider })));
        spinner.start();
      }
      ...
      if (!apiKey) {
        ...
        if (spinner) {
          spinner.info(chalk.blue('🧠 LLM semantic analysis enabled (legacy mode)'));
          spinner.start();
        }
      }
    }

    validator = new MCPValidator(transport, options.config as string | undefined, {
      enableSemanticCheck,
      llmProvider,
    ...
    if (loadError) {
      ...
      if (loadError instanceof ZodError) {
        loadError.issues.forEach((issue) => {
           if (spinner) spinner.info(chalk.dim(`   • ${issue.path.join('.')}: ${issue.message}`));
           else log.log(chalk.dim(`   • ${issue.path.join('.')}: ${issue.message}`));
        });
      } else {
      ...
      if (spinner) {
        spinner.info(chalk.blue(t('config_using_path', { path: relativePath })));
        spinner.start();
      } else if (!isQuiet) {
    ...
    if (!handshakeResult.success) {
      ...
      log.log(chalk.dim(t('example_validate_command') + ' ' + target + ' --transport stdio'));
      log.log('');
    ...
    const modulesExecuted: Array<'security' | 'quality' | 'fuzzing' | 'protocol'> = ['security', 'quality'];
    ...
    const outputDir = path.resolve(PathValidator.validateOutputPath(rawOutputDir, '.'));
    ...
    const savedReports = await ReportingService.saveReport({ kind: 'validation', data: report }, {
      outputDir,
      formats,
      language: lang,
    ...
    for (const error of savedReports.errors) {
      if (spinner) spinner.warn(chalk.yellow(`Could not generate ${error.format}: ${error.message}`));
    }
    ...
    log.log(`${t('status')}: ${report.status === 'valid' ? chalk.green('✓ ' + t('status_valid')) : chalk.red('✗ ' + t('status_invalid'))}`);
    log.log(`${t('protocol')}: ${report.protocol_version}`);
    log.log(`${t('tools')}: ${report.tools.count} (${chalk.green(report.tools.valid)} ${t('valid_label')})`);
    ...
    if (report.protocolCompliance) {
      ...
      if (!pc.passed) {
        ...
        pc.issues.forEach(i => log.log(chalk.red(`  - [${i.code}] ${i.message}`)));
      }
    }
    ...
    if (report.quality.issues.length > 0) {
      log.log(chalk.yellow(`  ${report.quality.issues.length} ${t('semantic_issues')}`));
    }

    // Fuzzing
    if (report.fuzzing) {
      log.log(`${t('fuzzing_label')}: ${chalk.cyan(report.fuzzing.totalTests)} ${t('tests_label')}`);
      if (report.fuzzing.crashes > 0) {
        log.log(chalk.bgRed.white.bold(`  ${t('crashes_detected')}: ${report.fuzzing.crashes}  `));
      } else if (report.fuzzing.failedTests > 0) {
        log.log(chalk.yellow(`  ${t('failures_label')}: ${report.fuzzing.failedTests}`));
      } else {
    ...
    log.log(`${t('score')}: ${scoreColor(report.security.score + '/100')} (${scoreColor(report.security.level)})`);
    ...
    log.log(`${chalk.bold(t('markdown_label'))}: ${chalk.magenta(mdReportPath)}`);
    if (htmlReportPath) log.log(`${t('html_label')}: ${chalk.blue(htmlReportPath)}`);
    ...
    if (llmUsed) {
      log.log(chalk.dim('🤖 ' + t('disclaimer_llm_notice').replace('{provider}', llmProvider || '')));
    }
    log.log('');
    ...
    if (options.compareBaseline) {
      ...
      if (!baseline) {
        ...
        log.log(chalk.dim(t('baseline_not_found_tip', { path: String(options.compareBaseline) })));
      } else {
        const comparison = BaselineManager.compare(report, baseline);

        // Save comparison report
        const comparisonPath = path.join(comparisonDir, `mcp-comparison-${savedReports.timestamp}.json`);
        ...
        log.log('');
        log.log(`${t('security_audit')} ${t('score')}: ${baseline.securityScore} → ${comparison.current.securityScore} ${comparison.delta.securityScore >= 0 ? chalk.green(`(+${comparison.delta.securityScore})`) : chalk.red(`(${comparison.delta.securityScore})`)}`);
        log.log(`${t('quality')} ${t('score')}:  ${baseline.qualityScore} → ${comparison.current.qualityScore} ${comparison.delta.qualityScore >= 0 ? chalk.green(`(+${comparison.delta.qualityScore})`) : chalk.red(`(${comparison.delta.qualityScore})`)}`);

        if (comparison.delta.newCriticalFindings > 0) {
          log.log(chalk.red.bold(`⚠️  ${t('baseline_new_critical', { count: comparison.delta.newCriticalFindings })}`));
        }
        if (comparison.delta.newHighFindings > 0) {
          log.log(chalk.yellow(`⚠️  ${t('baseline_new_high', { count: comparison.delta.newHighFindings })}`));
        }
        if (comparison.delta.fixedFindings > 0) {
          log.log(chalk.green(`✓ ${t('baseline_findings_fixed', { count: comparison.delta.fixedFindings })}`));
        }
    ...
    if (shouldFailFromBaseline) {
      ...
      const criticalFindingsCount = report.security?.findings 
        ? report.security.findings.filter(f => f.severity === 'critical').length 
        : 0;
    ...
    log.error(chalk.red(t('error') + ': ') + (error instanceof Error ? error.message : String(error)));
    log.log('');
    ...
    if (errorMsg.includes('econnrefused') || errorMsg.includes('connection refused')) {
      ...
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_verify_port') + '\n'));
    } else if (errorMsg.includes('timeout')) {
      log.log(chalk.gray('• ') + t('server_slow'));
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_increase_timeout')));
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_check_logs') + '\n'));
    } else if (errorMsg.includes('enotfound') || errorMsg.includes('dns')) {
      log.log(chalk.gray('• ') + t('dns_error'));
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_check_spelling')));
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_ping_hostname') + '\n'));
    } else if (errorMsg.includes('parse') || errorMsg.includes('json')) {
      log.log(chalk.gray('• ') + t('invalid_json'));
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_check_implementation')));
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_use_verbose_raw') + '\n'));
    } else {
      log.log(chalk.gray('• ') + t('unexpected_error'));
      log.log(chalk.dim('  ' + t('try_prefix') + ' ' + t('tip_verbose')));
      log.log(chalk.dim('  ' + t('try_prefix') + ' mcp-verify doctor ' + target + '\n'));
    }

    log.log(chalk.bold('🔍 ' + t('need_help') + ''));
    log.log(chalk.gray('   ' + t('label_run') + ' ') + chalk.cyan('mcp-verify doctor ' + target));
    log.log(chalk.gray('   ' + t('label_docs') + ' ') + chalk.cyan('mcp-verify examples'));
    log.log(chalk.gray('   ' + t('label_issues') + ' ') + chalk.cyan('https://github.com/FinkTech/mcp-verify/issues\n'));
    return 1;
  } finally {
    if (validator) await validator.cleanup();
  }
