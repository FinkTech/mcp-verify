...
function isValidInputSchema(
  schema: unknown,
): schema is { properties?: Record<string, unknown> } {
  return typeof schema === "object" && schema !== null;
}

function isValidPromptArgument(
  arg: unknown,
): arg is { name: string; required?: boolean; description?: string } {
  return (
    typeof arg === "object" &&
    arg !== null &&
    "name" in arg &&
...
export async function runPlaygroundAction(
  ...
  try {
    ...
    allItems.forEach((item, i) => {
      const typeLabel =
        item.type === "tool"
          ? chalk.cyan("[" + t("tool_label") + "]")
          : chalk.magenta("[" + t("prompt_label") + "]");
      console.log(
        `  ${chalk.white(i + 1)}. ${typeLabel} ${chalk.bold(item.data.name)} - ${chalk.gray(item.data.description || t("no_description"))}`,
      );
    });
    console.log("");
    ...
    while (true) {
        ...
        try {
          ...
          if (isTool) {
            ...
            if (
              ...
              isValidInputSchema(toolData.inputSchema) &&
      ...
      if (result.success) {
        console.log(
          chalk.green("✓ " + t("success_label")) + ` (${result.durationMs}ms)`,
        );
