 ...
 * SEC-050: Missing Audit Logging Detection Test
 */
...
describe('SEC-050: Missing Audit Logging Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-050');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'missing-audit', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect missing audit logging (SEC-050)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-050');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/audit|log|privileged|action/);
    console.log('\n✓ SEC-050 detected:', finding.message);
  });
});
