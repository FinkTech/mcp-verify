 ...
 * SEC-035: Agent Identity Spoofing Detection Test
 */
...
describe('SEC-035: Agent Identity Spoofing Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-035');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'identity-spoofing', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect agent identity spoofing (SEC-035)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-035');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/spoof|impersonate|identity|agent/);
    console.log('\n✓ SEC-035 detected:', finding.message);
  });
});
