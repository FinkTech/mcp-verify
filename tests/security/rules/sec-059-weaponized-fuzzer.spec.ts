 ...
 * SEC-059: Weaponized MCP Fuzzer Detection Test
 */
...
describe('SEC-059: Weaponized MCP Fuzzer Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-059');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'weaponized-fuzzer', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect weaponized mcp fuzzer (SEC-059)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-059');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/weapon|fuzzer|attack|target/);
    console.log('\n✓ SEC-059 detected:', finding.message);
  });
});
