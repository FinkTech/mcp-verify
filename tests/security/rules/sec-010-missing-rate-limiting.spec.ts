 ...
 * SEC-010: Missing Rate Limiting Detection Test
 */
...
describe('SEC-010: Missing Rate Limiting Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-010');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'missing-rate-limit', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect missing rate limiting (SEC-010)', async () => {
