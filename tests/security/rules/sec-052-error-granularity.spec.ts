 ...
 * SEC-052: Insufficient Error Granularity Detection Test
 */
...
describe('SEC-052: Insufficient Error Granularity Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-052');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'error-granularity', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect insufficient error granularity (SEC-052)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-052');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/error|granular|implementation|detail/);
    console.log('\n✓ SEC-052 detected:', finding.message);
  });
});
