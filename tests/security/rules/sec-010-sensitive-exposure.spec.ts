 ...
 * SEC-010: Sensitive Data Exposure Detection Test
 */
...
describe('SEC-010: Sensitive Data Exposure Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-010');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'sensitive-exposure', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect sensitive data exposure (SEC-010)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-010');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical/i);
    expect(finding.message.toLowerCase()).toMatch(/sensitive|credential|api.?key|secret|password/);
    console.log('\n✓ SEC-010 detected:', finding.message);
  });
});
