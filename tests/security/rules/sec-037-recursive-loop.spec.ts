 ...
 * SEC-037: Recursive Agent Loop Detection Test
 */
...
describe('SEC-037: Recursive Agent Loop Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-037');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'recursive-loop', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect recursive agent loop (SEC-037)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-037');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/recursive|loop|depth|limit/);
    console.log('\n✓ SEC-037 detected:', finding.message);
  });
});
