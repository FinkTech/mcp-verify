 ...
 * SEC-039: Excessive Data Disclosure Detection Test
 */
...
describe('SEC-039: Excessive Data Disclosure Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-039');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'excessive-disclosure', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect excessive data disclosure (SEC-039)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-039');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/excessive|disclosure|pii|pagination/);
    console.log('\n✓ SEC-039 detected:', finding.message);
  });
});
