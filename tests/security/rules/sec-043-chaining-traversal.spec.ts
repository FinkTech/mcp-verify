 ...
 * SEC-043: Tool Chaining Path Traversal Detection Test
 */
...
describe('SEC-043: Tool Chaining Path Traversal Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-043');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'chaining-traversal', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect tool chaining path traversal (SEC-043)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-043');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/chain|traversal|read|execute/);
    console.log('\n✓ SEC-043 detected:', finding.message);
  });
});
