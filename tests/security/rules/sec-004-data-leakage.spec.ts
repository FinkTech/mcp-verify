 ...
 * SEC-004: Data Leakage Detection Test
 */
...
describe('SEC-004: Data Leakage Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-004');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'data-leakage', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect data leakage vulnerability (SEC-004)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-004');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/data|leak|environment|debug/);
    console.log('\n✓ SEC-004 detected:', finding.message);
  });
});
