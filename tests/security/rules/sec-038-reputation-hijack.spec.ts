 ...
 * SEC-038: Agent Reputation Hijacking Detection Test
 */
...
describe('SEC-038: Agent Reputation Hijacking Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-038');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'reputation-hijack', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect agent reputation hijacking (SEC-038)', async () => {
