 ...
 * SEC-044: Distributed Agent DDoS Detection Test
 */
...
describe('SEC-044: Distributed Agent DDoS Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-044');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'distributed-ddos', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect distributed agent ddos (SEC-044)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-044');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/ddos|broadcast|distributed|concurrency/);
    console.log('\n✓ SEC-044 detected:', finding.message);
  });
});
