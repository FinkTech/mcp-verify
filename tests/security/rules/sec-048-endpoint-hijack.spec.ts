 ...
 * SEC-048: API Endpoint Hijacking Detection Test
 */
...
describe('SEC-048: API Endpoint Hijacking Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-048');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'endpoint-hijack', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect api endpoint hijacking (SEC-048)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-048');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/endpoint|hijack|collision|register/);
    console.log('\n✓ SEC-048 detected:', finding.message);
  });
});
