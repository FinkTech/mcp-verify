 ...
 * SEC-029: Missing CORS Validation Detection Test
 */
...
describe('SEC-029: Missing CORS Validation Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-029');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'missing-cors', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect missing cors validation (SEC-029)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-029');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/cors|origin|cross.?origin/);
    console.log('\n✓ SEC-029 detected:', finding.message);
  });
});
