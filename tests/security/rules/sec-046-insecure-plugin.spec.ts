 ...
 * SEC-046: Insecure Plugin Design Detection Test
 */
...
describe('SEC-046: Insecure Plugin Design Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-046');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'insecure-plugin', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect insecure plugin design (SEC-046)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-046');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/plugin|signature|sandbox|load/);
    console.log('\n✓ SEC-046 detected:', finding.message);
  });
});
