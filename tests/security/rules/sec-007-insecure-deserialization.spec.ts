 ...
 * SEC-007: Insecure Deserialization Detection Test
 */
...
describe('SEC-007: Insecure Deserialization Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-007');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'insecure-deserialization', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect insecure deserialization (SEC-007)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-007');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/deserializ|eval|unsafe/);
    console.log('\n✓ SEC-007 detected:', finding.message);
  });
});
