 ...
 * SEC-008: ReDoS Detection Detection Test
 */
...
describe('SEC-008: ReDoS Detection Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-008');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'redos', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect redos detection (SEC-008)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-008');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/redos|regex|catastrophic|backtrack/);
    console.log('\n✓ SEC-008 detected:', finding.message);
  });
});
