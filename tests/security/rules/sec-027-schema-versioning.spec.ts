 ...
 * SEC-027: Schema Versioning Absent Detection Test
 */
...
describe('SEC-027: Schema Versioning Absent Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-027');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'schema-versioning', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect schema versioning absent (SEC-027)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-027');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/schema|version|legacy|compatibility/);
    console.log('\n✓ SEC-027 detected:', finding.message);
  });
});
