 ...
 * SEC-053: Insufficient Output Entropy Detection Test
 */
...
describe('SEC-053: Insufficient Output Entropy Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-053');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'output-entropy', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect insufficient output entropy (SEC-053)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-053');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/entropy|token|timestamp|predictable/);
    console.log('\n✓ SEC-053 detected:', finding.message);
  });
});
