...
describe("SEC-004: SSRF Detection", () => {
  ...
  it("should detect SSRF vulnerability (SEC-004) via validate command", async () => {
    ...
    expect(ssrfFinding.message.toLowerCase()).toMatch(
      /url input without validation|ssrf|server-side request forgery|internal network|localhost|192\.168/,
    );
  ...
  it("should detect SSRF in tool description containing internal IP patterns", async () => {
    ...
    const fetchUrlTool = report.tools.items.find(
      (t: any) => t.name === "fetch_url",
    );

    expect(fetchUrlTool).toBeDefined();
    expect(fetchUrlTool.description.toLowerCase()).toMatch(
      /internal|localhost|192\.168|private/,
    );
