 ...
 * SEC-012: Authentication Bypass Detection Test
 */
...
describe('SEC-012: Authentication Bypass Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-012');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'auth-bypass', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect authentication bypass (SEC-012)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-012');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical/i);
    expect(finding.message.toLowerCase()).toMatch(/auth|bypass|password|insecure|check/);
    console.log('\n✓ SEC-012 detected:', finding.message);
  });
});
