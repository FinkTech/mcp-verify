 ...
 * SEC-032: Model DoS via Tools Detection Test
 */
...
describe('SEC-032: Model DoS via Tools Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-032');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'model-dos', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect model dos via tools (SEC-032)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-032');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/dos|denial|massive|output|1gb/);
    console.log('\n✓ SEC-032 detected:', finding.message);
  });
});
