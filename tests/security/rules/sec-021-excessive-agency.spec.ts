 ...
 * SEC-021: Excessive Agency Detection Test
 */
...
describe('SEC-021: Excessive Agency Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-021');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'excessive-agency', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect excessive agency (SEC-021)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-021');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical/i);
    expect(finding.message.toLowerCase()).toMatch(/excessive|agency|delete|dangerous|confirmation/);
    console.log('\n✓ SEC-021 detected:', finding.message);
  });
});
