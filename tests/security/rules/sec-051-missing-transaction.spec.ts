 ...
 * SEC-051: Missing Transaction Semantics Detection Test
 */
...
describe('SEC-051: Missing Transaction Semantics Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-051');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'missing-transaction', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect missing transaction semantics (SEC-051)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-051');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/transaction|rollback|atomic|multi.?step/);
    console.log('\n✓ SEC-051 detected:', finding.message);
  });
});
