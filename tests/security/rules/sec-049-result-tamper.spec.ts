 ...
 * SEC-049: Tool Result Tampering Detection Test
 */
...
describe('SEC-049: Tool Result Tampering Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-049');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'result-tamper', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect tool result tampering (SEC-049)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-049');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/tamper|modify|result|signature/);
    console.log('\n✓ SEC-049 detected:', finding.message);
  });
});
