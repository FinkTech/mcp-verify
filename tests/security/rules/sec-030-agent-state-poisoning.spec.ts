 ...
 * SEC-030: Agent State Poisoning Detection Test
 */
...
describe('SEC-030: Agent State Poisoning Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-030');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'agent-state-poisoning', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect agent state poisoning (SEC-030)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-030');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/state|poison|global|config|isolation/);
    console.log('\n✓ SEC-030 detected:', finding.message);
  });
});
