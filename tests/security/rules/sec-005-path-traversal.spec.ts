 ...
 * SEC-005: Path Traversal Detection Test
 */
...
describe('SEC-005: Path Traversal Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-005');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'path-traversal', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect path traversal vulnerability (SEC-005)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-005');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical/i);
    expect(finding.message.toLowerCase()).toMatch(/path|traversal|\.\.\/|directory/);
    console.log('\n✓ SEC-005 detected:', finding.message);
  });
});
