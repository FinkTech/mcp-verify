 ...
 * SEC-033: Training Data Poisoning Detection Test
 */
...
describe('SEC-033: Training Data Poisoning Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-033');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'training-poison', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect training data poisoning (SEC-033)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-033');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/training|poison|feedback|malicious/);
    console.log('\n✓ SEC-033 detected:', finding.message);
  });
});
