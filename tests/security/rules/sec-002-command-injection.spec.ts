 ...
 * SEC-002: Command Injection Detection Test
 */
...
describe('SEC-002: Command Injection Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-002');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'command-injection', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect command injection vulnerability (SEC-002)', async () => {
