 ...
 * SEC-055: Jailbreak as Service Detection Test
 */
...
describe("SEC-055: Jailbreak as Service Detection", () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(
    __dirname,
    "../../__test-reports__/sec-055",
  );

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({
      profile: "jailbreak-service",
      lang: "en",
      transport: "stdio",
      timeout: 30000,
    });
  ...
  it("should detect jailbreak as service (SEC-055)", async () => {
