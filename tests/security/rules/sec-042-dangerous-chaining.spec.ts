 ...
 * SEC-042: Dangerous Tool Chaining Detection Test
 */
...
describe('SEC-042: Dangerous Tool Chaining Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-042');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'dangerous-chaining', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect dangerous tool chaining (SEC-042)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-042');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/chain|pipe|shell|sanitiz/);
    console.log('\n✓ SEC-042 detected:', finding.message);
  });
});
