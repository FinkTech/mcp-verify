 ...
 * SEC-054: Timing Side Channel Auth Detection Test
 */
...
describe('SEC-054: Timing Side Channel Auth Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-054');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'timing-side-channel', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect timing side channel auth (SEC-054)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-054');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/timing|side.?channel|constant.?time|compare/);
    console.log('\n✓ SEC-054 detected:', finding.message);
  });
});
