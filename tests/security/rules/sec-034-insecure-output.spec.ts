 ...
 * SEC-034: Insecure Output Handling Detection Test
 */
...
describe('SEC-034: Insecure Output Handling Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-034');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'insecure-output', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect insecure output handling (SEC-034)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-034');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/output|xss|escape|render|html/);
    console.log('\n✓ SEC-034 detected:', finding.message);
  });
});
