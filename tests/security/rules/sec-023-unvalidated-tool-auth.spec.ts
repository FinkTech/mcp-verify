 ...
 * SEC-023: Unvalidated Tool Authorization Detection Test
 */
...
describe('SEC-023: Unvalidated Tool Authorization Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-023');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'unvalidated-tool-auth', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect unvalidated tool authorization (SEC-023)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-023');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/authorization|permission|privilege|tool/);
    console.log('\n✓ SEC-023 detected:', finding.message);
  });
});
