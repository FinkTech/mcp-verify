 ...
 * SEC-026: Missing Capability Negotiation Detection Test
 */
...
describe('SEC-026: Missing Capability Negotiation Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-026');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'missing-capability', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect missing capability negotiation (SEC-026)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-026');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/capability|negotiat|feature|client/);
    console.log('\n✓ SEC-026 detected:', finding.message);
  });
});
