...
describe("SEC-019: Missing Input Constraints Detection", () => {
  const baseTestReportDir = path.resolve(
    __dirname,
    "../../__test-reports__/sec-019",
  );
  ...
  it("should detect missing input constraints on a vulnerable server (SEC-019)", async () => {
    ...
    try {
      await serverManager.start({
        profile: "missing-input-constraints",
        lang: "en",
        transport: "stdio",
        timeout: 30000,
      });
      ...
      finding = report.security.findings.find(
        (f: any) => f.ruleCode === "SEC-019",
      );
    } finally {
    ...
    expect(finding.message).toMatch(
      /Parameter data in tool process_data lacks maxLength constraint/,
    );
