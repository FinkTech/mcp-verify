...
describe('SEC-009: Weak Authentication Detection', () => {
  ...
  beforeAll(async () => {
    ...
    await serverManager.start({ profile: 'weak-auth', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect weak authentication (SEC-009)', async () => {
    ...
    expect(finding.message.toLowerCase()).toMatch(/auth|brute.?force|rate.?limit|login/);
