 ...
 * SEC-047: Insecure Default Configuration Detection Test
 */
...
describe("SEC-047: Insecure Default Configuration Detection", () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(
    __dirname,
    "../../__test-reports__/sec-047",
  );

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({
      profile: "insecure-defaults",
      lang: "en",
      transport: "stdio",
      timeout: 30000,
    });
  ...
  it("should detect insecure default configuration (SEC-047)", async () => {
