 ...
 * SEC-031: Cross-Agent Prompt Injection Detection Test
 */
...
describe('SEC-031: Cross-Agent Prompt Injection Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-031');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'cross-agent-injection', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect cross-agent prompt injection (SEC-031)', async () => {
    ...
    const finding = report.security.findings.find((f: any) => f.ruleCode === 'SEC-031');
    expect(finding).toBeDefined();
    expect(finding.severity).toMatch(/high|critical|medium/i);
    expect(finding.message.toLowerCase()).toMatch(/cross.?agent|inject|message|unsanit/);
    console.log('\n✓ SEC-031 detected:', finding.message);
  });
});
