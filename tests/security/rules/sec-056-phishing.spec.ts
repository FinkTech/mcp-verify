 ...
 * SEC-056: Phishing via MCP Detection Test
 */
...
describe('SEC-056: Phishing via MCP Detection', () => {
  let serverManager: TestServerManager;
  const testReportDir = path.resolve(__dirname, '../../__test-reports__/sec-056');

  beforeAll(async () => {
    serverManager = new TestServerManager();
    await serverManager.start({ profile: 'phishing', lang: 'en', transport: 'stdio', timeout: 30000 });
  ...
  it('should detect phishing via mcp (SEC-056)', async () => {
