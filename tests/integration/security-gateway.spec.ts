...
import { McpProxyServer, ProxyAuditEvent } from '@mcp-verify/core/use-cases/proxy/proxy-server';
...
describe('Security Gateway - 3-Layer Defense Architecture', () => {
  ...
  async function callProxy(method: string, params: any): Promise<JsonRpcResponse> {
  ...
  describe('Layer 3: LLM Rules - Deep Analysis (Opt-in)', () => {
    ...
    async function callDeepProxy(method: string, params: any): Promise<JsonRpcResponse> {
      const response = await fetch(`http://localhost:${DEEP_ANALYSIS_PORT}/message`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          jsonrpc: '2.0',
  ...
  describe('Explainable Blocking: Structured Metadata', () => {
    ...
    it('should provide actionable remediation guidance', async () => {
      ...
      if (response.error?.data?.remediation) {
        ...
        expect(response.error.data.remediation).toMatch(/sanitiz|validat|escap|avoid|use|instead/i);
      }
      expect(true).toBe(true);
    });
  });
  ...
  describe('Audit Events: Security Analysis Emission', () => {
    ...
    it('should include findings in security-analysis event', async () => {
      ...
      const securityEvent = auditEvents.find(e => e.type === 'security-analysis');
      if (securityEvent && 'findings' in securityEvent && securityEvent.result === 'blocked') {
  ...
  describe('Performance: Latency Requirements', () => {
    ...
    it('Layer 1+2 should complete in <100ms for suspicious tools', async () => {
      ...
      const securityEvent = auditEvents.find(e => e.type === 'security-analysis');
      if (securityEvent && 'latencyMs' in securityEvent && 'layer' in securityEvent) {
