...
const filePath = path.resolve(__dirname, '../../tests/fixtures/vulnerable_servers/configurable-server.ts');
...
content = content.replace(/path-traversal: SEC-005/g, 'path-traversal: SEC-007');
content = content.replace(/xxe: SEC-006/g, 'xxe: SEC-005');
content = content.replace(/insecure-deserialization: SEC-007/g, 'insecure-deserialization: SEC-006');
content = content.replace(/redos: SEC-008/g, 'redos: SEC-011');
content = content.replace(/sensitive-exposure: SEC-010/g, 'sensitive-exposure: SEC-009');
...
if (content.includes("'redos': [") && !content.includes("pattern: '^(a+)+$'")) {
    content = content.replace(/'redos': \[\s*{\s*name: 'validate_input'/, `'redos': [
    {
      name: 'validate_input'`);
    
    content = content.replace(/description: isEs \? 'Input a validar con regex vulnerable' : 'Input to validate with vulnerable regex',/, `description: isEs ? 'Input a validar con regex vulnerable' : 'Input to validate with vulnerable regex',
            pattern: '^(a+)+$',`);
