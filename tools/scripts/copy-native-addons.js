...
const verbose = (msg) => {
  if (VERBOSE) console.log(`  ${c.dim("[verbose] " + msg)}`);
};
const die = (msg) => {
  console.error(`\n  ${c.red("✗")} ${c.bold(msg)}\n`);
  process.exit(1);
...
const NATIVE_ADDONS = [
  {
    ...
    files: [
      ...
      {
        resolveFrom: "@napi-rs/keyring/keyring.linux-x64-gnu.node",
        destName: "keyring.linux-x64-gnu.node",
        targetPlatform: "linux",
      },
      // linux-x64 (musl) — Alpine Linux / Docker slim images
      {
        resolveFrom: "@napi-rs/keyring/keyring.linux-x64-musl.node",
        destName: "keyring.linux-x64-musl.node",
        targetPlatform: "linux",
      },
      // macOS x64 (Intel)
      {
        resolveFrom: "@napi-rs/keyring/keyring.darwin-x64.node",
        destName: "keyring.darwin-x64.node",
        targetPlatform: "macos",
      },
      // macOS arm64 (Apple Silicon) — cross-compiled or native M1/M2 CI
      {
        resolveFrom: "@napi-rs/keyring/keyring.darwin-arm64.node",
        destName: "keyring.darwin-arm64.node",
        targetPlatform: "macos",
      },
      // Windows x64 (MSVC)
      {
        resolveFrom: "@napi-rs/keyring/keyring.win32-x64-msvc.node",
        destName: "keyring.win32-x64-msvc.node",
  ...
  },
  {
    ...
    files: [
      {
        resolveFrom: "fsevents/fsevents.node",
        destName: "fsevents.node",
        targetPlatform: "macos",
      },
    ],
  },
...
function copyAddon({ srcPath, destDir, destName, pkg, optional }) {
  ...
  try {
    ...
    success(
      `${c.bold(pkg)} → ${relDest}  ${c.dim("(" + (fs.statSync(destPath).size / 1024).toFixed(0) + " KB)")}`,
    );
    return true;
  } catch (err) {
    if (optional) {
...
function writeAddonManifest(targetDir, copiedFiles) {
  ...
  if (!DRY_RUN) {
    ...
    log(
      `${c.dim("[dry-run]")} would write manifest: ${path.relative(ROOT, manifestPath)}`,
    );
  }
}
...
function main() {
  ...
  if (DRY_RUN)
    console.log(
      c.yellow("  Running in dry-run mode — no files will be written.\n"),
    );
  ...
  for (const platform of availablePlatforms) {
    ...
    for (const addon of NATIVE_ADDONS) {
      ...
      if (!addon.platforms.includes(platform)) {
        verbose(
          `Skipping ${addon.pkg} for ${platform} (not in platforms list)`,
        );
        continue;
      }

      // Find files for this specific platform
      const platformFiles = addon.files.filter(
        (f) => f.targetPlatform === platform,
      );
      ...
      for (const addonFile of platformFiles) {
        ...
        if (!srcPath) {
          if (addon.optional) {
            warn(
              `${addon.pkg}/${addonFile.destName} not installed — skipping (optional)`,
            );
            totalSkipped++;
          } else {
            die(
  ...
  if (totalSkipped > 0)
    warn(`${totalSkipped} optional addon(s) skipped (not installed)`);
