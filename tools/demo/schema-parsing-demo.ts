...
simplePayloads.slice(0, 5).forEach((payload, idx) => {
  console.log(`  ${idx + 1}. [${payload.severity.toUpperCase()}] ${payload.description}`);
...
nestedSamples.forEach((payload, idx) => {
  ...
  console.log(`     [${payload.severity.toUpperCase()}] ${payload.description}`);
  console.log(`     Nested structure: ${JSON.stringify(payload.value, null, 2).split('\n').slice(0, 5).join('\n     ')}`);
  console.log();
});
console.log();
...
console.log('     ✅ Structural attacks (missing fields, prototype pollution, DoS)');
console.log();
...
console.log('  • Prioritizes attacks by severity (critical > high > medium > low)');
console.log();
console.log('To test:');
