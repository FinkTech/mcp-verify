...
const tools = [
  ...
  {
    name: 'parse_xml',
    description: isEs ? 'Analizar documento XML (vulnerable a ataques XXE)' : 'Parse XML document (vulnerable to XXE attacks)',
    inputSchema: {
      type: 'object',
      properties: {
        xml: {
          type: 'string',
          description: isEs ? 'Documento XML a analizar' : 'XML document to parse'
        }
      }
    }
  },
  {
    name: 'deserialize',
    description: isEs ? 'Deserializar objeto desde una cadena JSON usando eval()' : 'Deserialize object from JSON string using eval()',
    inputSchema: {
      type: 'object',
      properties: {
        data: {
