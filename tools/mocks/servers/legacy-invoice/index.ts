  ...
  try {
    ...
    for (const year of years) {
      for (const dept of depts) {
        await fs.mkdir(path.join(INVOICE_BASE_DIR, year, dept), {
          recursive: true,
        });
      }
    }
...
async function searchInvoices(
  args: z.infer<typeof SearchInvoicesSchema>,
): Promise<string> {
  try {
...
const tools: Tool[] = [
  {
    name: "list_invoices",
    description:
      "Lists invoices in the archive system. Optionally filter by year and department.",
    inputSchema: {
      type: "object",
      properties: {
        year: {
        ...
        department: {
          type: "string",
          description:
            "Filter by department (e.g., 'logistics', 'procurement')",
        },
      },
    },
  },
  {
    name: "get_invoice",
    description:
      "Retrieves the content of a specific invoice file by its path relative to the archive root.",
    inputSchema: {
      type: "object",
      properties: {
        invoice_path: {
          type: "string",
          description:
            "Relative path to the invoice file (e.g., '2024/logistics/INV-2024-001.json')",
        },
      },
  ...
  },
  {
    ...
    description:
      "Searches for invoices matching specific criteria across the archive.",
    inputSchema: {
      type: "object",
      properties: {
        query: {
